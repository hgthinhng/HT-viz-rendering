...
def esc(text):
    """Escape the 4 XML special characters."""
    return text.replace("&", "&amp;").replace("<", "&lt;").replace(">", "&gt;").replace('"', "&quot;")

def rpr(font="Inter", sz=20, bold=False, italic=False, color="1F1F1F", shd_fill=None,
...
def render_subscript_text(text, font="Consolas", sz=26, color="4338CA", bold=False):
    """
...
def render_subscript_colored(text, color, font="Consolas", sz=26, bold=False):
...
def parse_inline(text):
    ...
    while remaining:
        ...
        if m:
            ...
            runs.append(run(term, font="Inter", sz=20, bold=True, color=C["term_teal"]))
            # Meaning: italic, gray-muted, 1 size smaller — exactly per user spec
            runs.append(run(f" (", font="Inter", sz=18, italic=True, color=C["muted"]))
            runs.append(run(meaning, font="Inter", sz=18, italic=True, color=C["muted"]))
            runs.append(run(")", font="Inter", sz=18, italic=True, color=C["muted"]))
        ...
        if m:
            runs.append(run(m.group(1), font="Inter", sz=20, bold=True, color=C["term_teal"]))
            remaining = remaining[m.end():]
            continue
        ...
        if m:
            runs.append(run(m.group(1), font="Inter", sz=20, color=C["body"], shd_fill=C["highlight_bg"]))
            remaining = remaining[m.end():]
            continue
        ...
        if m:
            runs.append(render_subscript_text(m.group(1), font="Consolas", sz=20, color=C["indigo"]))
            remaining = remaining[m.end():]
            continue
...
def _plain_text_runs(text):
    ...
    has_list = re.search(r'\(\d+\)\s*\S+', text)
    if not has_list:
        if text:
            ...
            while remaining:
                ...
                next_marker = -1
                marker_type = None
                if m_ct >= 0 and (m_vd < 0 or m_ct < m_vd):
                    next_marker = m_ct
                    marker_type = 'ct'
                elif m_vd >= 0:
                    next_marker = m_vd
            ...
            for seg_text, kind in segments:
                ...
                if kind == 'comthuc':
                    ...
                    result.append(run(seg_text, font="Inter", sz=20, italic=True, color=C["dark_amethyst"], letter_spacing=10))
                elif kind == 'vidu':
                    # "Ví dụ N.M": italic + dark olive gold (distinct from icon_gold), no bold no enlarge
                    result.append(run(seg_text, font="Inter", sz=20, italic=True, color=C["dark_olive_gold"], letter_spacing=10))
                else:
                    ...
                    if re.search(r'[A-Za-z]+_\{[^}]+\}|[A-Za-z]+_[A-Za-z0-9]+|[A-Za-z\)]\^\{[^}]+\}|[A-Za-z\)]\^-?[A-Za-z0-9]+|\\[A-Za-z]+', seg_text):
                        result.append(render_subscript_text(seg_text, font="Inter", sz=20, color=C["body"]))
                    else:
                        result.append(run(seg_text, font="Inter", sz=20, color=C["body"], letter_spacing=BODY_TRACKING))
        return result
        ...
        if i + 1 < len(parts) and parts[i + 1].isdigit():
            ...
            if before:
                ...
                if first_item and before.rstrip().endswith(':'):
                    ...
                    if prior_text:
                        result.append(run(prior_text + " ", font="Inter", sz=20, color=C["body"], letter_spacing=BODY_TRACKING))
                    intro_rpr_inner = (
                        f'<w:rFonts w:ascii="Inter" w:hAnsi="Inter" w:cs="Inter" w:eastAsia="Inter"/>'
                        f'<w:b/><w:bCs/>'
                        f'<w:u w:val="single"/>'
                    ...
                    result.append(run(before + " ", font="Inter", sz=20, color=C["body"], letter_spacing=BODY_TRACKING))
            num = int(parts[i + 1])
            ...
            result.append(run(glyph, font="Inter", sz=22, bold=True, color=C["section_purple"]))
            result.append(run("  ", font="Inter", sz=20, color=C["body"]))
            first_item = False
            i += 2
        else:
            text_part = parts[i].strip().rstrip(',').strip()
            if text_part:
                result.append(run(text_part, font="Inter", sz=20, color=C["body"], letter_spacing=BODY_TRACKING))
            i += 1
...
def render_end_paper(position="front"):
    ...
    for txt, sz in patterns:
        ...
        for i, (pattern_text, pattern_sz) in enumerate([(txt, sz)]):
            ...
            orn_run = run(pattern_text, font="Raleway", sz=pattern_sz, color=color, letter_spacing=80)
...
def render_cover(subject, title, subtitle, hero_prompt=""):
    ...
    5. "CFA LEVEL II" badge: Raleway italic, muted, letter-spaced ALL CAPS
    """
    paras = []
    ...
    subject_rpr = rpr(font="Raleway", sz=34, bold=True, color=subj_color, letter_spacing=80)
    paras.append(para(
    ...
    if hero_prompt or True:  # Always render placeholder for now
        ...
        label_run = run("HÌNH ẢNH", font="Raleway", sz=18, italic=True, color=C["muted"], letter_spacing=80)
        ...
        zwsp_run = run("\u200B", font="Inter", sz=10, color=C["page_bg"])
    ...
    if mod_match:
        ...
        num_rpr  = rpr(font="Raleway", sz=44, bold=True,  color=MODULE_NUM_COLOR)
        sep_rpr  = rpr(font="Raleway", sz=44, bold=False, color=C["muted"])
        name_rpr = rpr(font="Raleway", sz=44, bold=True,  color=MODULE_NAME_COLOR)
        title_xml = (
    ...
    # "CFA LEVEL II" badge: Raleway italic, muted, letter-spaced
    badge_rpr = rpr(font="Raleway", sz=18, italic=True, color=C["muted"], letter_spacing=40)
    paras.append(para(
        f'<w:r><w:rPr>{badge_rpr}</w:rPr><w:t xml:space="preserve">CFA LEVEL II</w:t></w:r>',
...
def render_toc():
    ...
    # TOC heading
    toc_heading = run("Mục lục", font="Raleway", sz=36, bold=True, color=C["section_purple"])
    ...
    toc_field = (
        ...
        '<w:r><w:rPr><w:rFonts w:ascii="Inter" w:hAnsi="Inter"/><w:i/><w:color w:val="6B6B6B"/><w:sz w:val="20"/></w:rPr>'
...
def render_formula_card(formulas):
    ...
    # Heading
    heading = run("Formula Card", font="Raleway", sz=36, bold=True, color=C["section_purple"])
    parts.append(para(heading, spacing_before=400, spacing_after=120, jc="center"))

    sub = run("Tổng hợp công thức của module", font="Inter", sz=18, italic=True, color=C["muted"])
    ...
    for idx, f_text in enumerate(formulas, 1):
        ...
        num_run = run(f"({idx})  ", font="Inter", sz=18, bold=True, color=C["section_num_orange"])
        # Formula main line
        f_main = f_text.split('\n')[0].strip() if '\n' in f_text else f_text.strip()
        f_runs = render_subscript_text(f_main, font="Consolas", sz=20, color=C["indigo"])
...
def render_glossary(terms_dict):
    ...
    heading = run("Thuật ngữ", font="Raleway", sz=36, bold=True, color=C["section_purple"])
    parts.append(para(heading, spacing_before=400, spacing_after=120, jc="center"))

    sub = run("Glossary, sắp theo bảng chữ cái", font="Inter", sz=18, italic=True, color=C["muted"])
    parts.append(para(sub, spacing_before=0, spacing_after=240, jc="center"))
    ...
    for term in sorted(terms_dict.keys(), key=lambda x: x.lower()):
        ...
        term_run = run(term, font="Inter", sz=20, bold=True, color=C["indigo"])
        sep_run = run("  :  ", font="Inter", sz=20, color=C["muted"])
        meaning_run = run(meaning, font="Inter", sz=20, italic=True, color=C["secondary"])
...
def render_halftitle(subject, title, module_num, module_name):
    ...
    name_run = run(module_name, font="Raleway", sz=44, italic=True,
    ...
    subj_run = run(f"Module {module_num}  \u00B7  {subject}", font="Raleway", sz=16,
...
def render_footnote(content):
    ...
    marker_run = run("\u2020 ", font="Inter", sz=14, color=C["icon_gold"])
...
def render_hanging_def(term, meaning):
    ...
    term_run = run(term, font="Inter", sz=20, bold=True, color=C["indigo"])
    sep_run = run("\u2003:\u2003", font="Inter", sz=20, color=C["muted"])
    meaning_run = run(meaning, font="Inter", sz=20, color=C["secondary"])
...
def render_colophon(subject, module_num, module_name):
    ...
    # Single fleuron
    fleuron_run = run("\u2766", font="Raleway", sz=48, color=C["gold"])
    ...
    lines = [
        ...
        ("", 12, "muted"),  # spacer
        (f"CFA Level II \u00B7 {subject} \u00B7 Module {module_num}", 16, "icon_gold"),
    ...
    for text, sz, color_key in lines:
        if not text:
            ...
            continue
        ln_run = run(text, font="Raleway", sz=sz, italic=True, color=C[color_key], letter_spacing=20)
...
def render_section(en_title, brief):
    ...
    num_run = run(f"{num}.\u2002", font="Inter", sz=32, bold=True, color=C["section_num_orange"], letter_spacing=20)
    section_colors = ["1E2862", "232D6E", "283278", "2C3878", "323D80", "384388", "3E4990", "445098"]
    sec_color = section_colors[(num - 1) % len(section_colors)]
    title_run = run(en_title, font="Inter", sz=32, bold=True, color=sec_color)
    paras.append(para(num_run + title_run, spacing_before=240, spacing_after=40))
...
def render_subsection(en_title, vi_title):
    ...
    title_run = run(en_title, font="Inter", sz=26, bold=True, color=C["section_purple"])
    ...
    if vi_title:
        ...
        vi_run = run(vi_title, font="Inter", sz=16, italic=True, color=C["muted"], letter_spacing=10)
...
def render_body_with_dropcap(text, standfirst=False):
    ...
    drop_para_xml = (
        ...
        f'</w:pPr>'
        f'<w:r><w:rPr>'
        f'<w:rFonts w:ascii="Raleway" w:hAnsi="Raleway" w:cs="Raleway" w:eastAsia="Raleway"/>'
        f'<w:b/><w:bCs/>'
...
def render_pullquote(content):
    ...
    quote_run = run(content.strip(), font="Inter", sz=28, italic=True, color=C["indigo"])
...
def render_runin(title, content):
    ...
    if not title_part.endswith('.'):
        title_part += '.'
    title_run = run(title_part + " ", font="Inter", sz=20, bold=True, color=C["section_purple"])
...
def render_section_ornament():
    ...
    ornament = "\u2756 \u2756 \u2756"  # ❖ ❖ ❖
    orn_run = run(ornament, font="Inter", sz=20, color=C["gold"], letter_spacing=80)
...
def render_table(title, header_cells, rows):
    ...
    if title:
        title_run = run(title, font="Inter", sz=18, italic=True, color=C["muted"], letter_spacing=15)
    ...
    if header_cells:
        ...
        for hc in header_cells:
            ...
            cell_run = run(hc.strip(), font="Inter", sz=18, bold=True, color="FFFFFF")
    ...
    for ri, row in enumerate(rows):
        ...
        for ci in range(n_cols):
            ...
            if cell_text:
                cell_run = render_subscript_text(cell_text, font="Inter", sz=18, color=C["body"])
            else:
                cell_run = run("", font="Inter", sz=18, color=C["body"])
...
def render_formula(content):
    ...
    for wl in where_lines:
        m = re.match(r'^((?:\\[A-Za-z]+)?[A-Za-z]*(?:_\{[^}]+\}|_[A-Za-z0-9]+)?(?:\^\{[^}]+\}|\^[A-Za-z0-9]+)?)\s*=', wl)
        if m:
    ...
    # Render main formula line(s): color-coded, centered within the indented zone
    for fl in formula_lines:
        formula_runs = _render_formula_line_colored(fl, var_color_map)
        paras.append(para(formula_runs, ppr_extra=formula_ppr, spacing_before=60, spacing_after=40, jc="center"))

    # Render each variable explanation with matching color
    for wl in where_lines:
        m = re.match(r'^((?:\\[A-Za-z]+)?[A-Za-z]*(?:_\{[^}]+\}|_[A-Za-z0-9]+)?(?:\^\{[^}]+\}|\^[A-Za-z0-9\*]+)?)\s*=\s*(.+)$', wl)
        if m:
            var_name = m.group(1).strip()
            description = m.group(2).strip()
            var_color = var_color_map.get(var_name, C["body"])

            var_runs = render_subscript_colored(var_name, color=var_color, font="Consolas", sz=18, bold=True)
            eq_run = run(" = ", font="Inter", sz=18, color=C["secondary"])

            if '|' in description:
                eng_part, vi_part = description.split('|', 1)
                # Process eng_part for Greek/sub/sup notation (handles \cdot, \theta, etc.)
                desc_run = render_subscript_text(eng_part.strip(), font="Inter", sz=18, color=C["secondary"])
                vi_run = run(f" ({vi_part.strip()})", font="Inter", sz=16, italic=True, color=C["muted"])
                line_runs = var_runs + eq_run + desc_run + vi_run
            else:
                desc_run = render_subscript_text(description, font="Inter", sz=18, color=C["secondary"])
                line_runs = var_runs + eq_run + desc_run

            paras.append(para(line_runs, ppr_extra=formula_ppr, spacing_before=20, spacing_after=20))
    ...
    zwsp_run = run("\u200B", font="Inter", sz=10, color=C["page_bg"])  # invisible spacer
...
def _render_formula_line_colored(formula_text, var_color_map):
    ...
    for token in tokens:
        ...
        if full_token_lookup in var_color_map:
            runs_xml += render_subscript_colored(token, color=var_color_map[full_token_lookup], font="Consolas", sz=26, bold=True)
        elif base_for_lookup in var_color_map:
            runs_xml += render_subscript_colored(token, color=var_color_map[base_for_lookup], font="Consolas", sz=26, bold=True)
        # Has subscript/superscript notation
        elif '_' in token or '^' in token or '\\' in token:
            runs_xml += render_subscript_colored(token, color=C["indigo"], font="Consolas", sz=26, bold=True)
        # Multi-letter word not in map
        elif re.match(r'^[A-Za-z\u0370-\u03FF]+$', token) and len(token) > 1:
            runs_xml += run(token, font="Consolas", sz=26, bold=True, color=C["section_purple"])
        # Single letter not in map
        elif re.match(r'^[A-Za-z\u0370-\u03FF]$', token):
            runs_xml += run(token, font="Consolas", sz=26, bold=True, color=C["secondary"])
        # Whitespace
        elif token.strip() == '':
            runs_xml += run(token, font="Consolas", sz=26, color=C["secondary"])
        # Operators and symbols: auto-convert * to multiplication sign
        else:
            display = token
            if token == '*':
                display = '\u00d7'
            runs_xml += run(display, font="Consolas", sz=26, color=C["secondary"])
...
def render_intuition(content):
    ...
    label_run = run("Trực giác:  ", font="Inter", sz=18, bold=True, italic=True, color=C["icon_gold"])
    body_runs = parse_inline(content.strip())
    # Make body italic
    import re as _re
    body_runs = _re.sub(r'<w:rPr>(?!.*<w:i/>)', '<w:rPr><w:i/><w:iCs/>', body_runs)
...
def render_check(content):
    ...
    label_run = run("\u2713 Đối chiếu.  ", font="Inter", sz=18, bold=True, color=C["icon_green"])
    body_runs = parse_inline(content.strip())
...
def render_diagram(diagram_type, params):
    ...
    cap_run = run(caption, font="Inter", sz=18, italic=True, color=C["muted"], letter_spacing=15)
    ...
    if diagram_type == "timeline":
        ...
        if events:
            ...
            for label, val in events:
                ...
                lbl_run = run(label, font="Inter", sz=16, bold=True, color=C["section_num_orange"])
                lbl_p = para(lbl_run, spacing_before=0, spacing_after=20, jc="center")
                val_runs = render_subscript_text(val.strip(), font="Inter", sz=18, color=C["body"])
        ...
        if center_text:
            ctr_run = run(center_text, font="Inter", sz=24, bold=True, color=C["section_purple"], letter_spacing=20)
            parts.append(para(ctr_run, spacing_before=80, spacing_after=120, jc="center"))
        if spokes:
            spoke_text = "  \u2756  ".join(spokes)
            sp_run = run(spoke_text, font="Inter", sz=18, italic=True, color=C["icon_gold"])
        ...
        if steps:
            joined = "  \u2192  ".join(steps)
            run_xml = render_subscript_text(joined, font="Inter", sz=20, color=C["body"])
        ...
        for row_pair in cells_data:
            ...
            for cell_text in row_pair:
                cell_runs = render_subscript_text(cell_text.strip(), font="Inter", sz=18, color=C["body"])
        ...
        run_xml = render_subscript_text(text, font="Inter", sz=20, italic=True, color=C["secondary"])
...
def render_epigraph(source, content):
    ...
    if source:
        ...
        attr_run = run(f", {source.strip()}", font="Inter", sz=16, italic=True,
...
def render_foldout(content):
    ...
    note_run = run("[Foldout: landscape orientation]", font="Inter", sz=14, italic=True, color=C["muted"])
...
def render_quote(source, content):
    ...
    open_q_run = run("\u201C", font="Raleway", sz=72, color=C["icon_gold"])
    ...
    # Attribution
    if source:
        attr_run = run(f"  {source.strip()}", font="Inter", sz=18, italic=True, color=C["icon_gold"], letter_spacing=15)
...
def render_margin(margin_type, content):
    ...
    label_run = run(label_text.upper(), font="Inter", sz=14, bold=True, italic=True,
...
def render_section_open(why_now, preview):
    ...
    def _pretext_runs(txt):
        ...
        if not ('_' in txt or '^' in txt or '\\' in txt):
            return run(txt, font="Inter", sz=20, italic=True, color=C["secondary"])
        out = ""
        for tok in re.split(r'(\s+)', txt):
            if not tok:
                continue
            if tok.isspace():
                out += run(tok, font="Inter", sz=20, italic=True, color=C["secondary"])
            elif '_' in tok or '^' in tok or '\\' in tok:
                out += render_subscript_text(tok, font="Consolas", sz=20, color=C["secondary"], bold=False)
            else:
                out += run(tok, font="Inter", sz=20, italic=True, color=C["secondary"])
        return out
...
def render_recap_handoff(recap, handoff):
    ...
    if recap:
        recap_run = run(recap.strip(), font="Inter", sz=20, italic=True, color=C["secondary"])
        parts.append(para(recap_run, spacing_before=120, spacing_after=40))

    if handoff:
        # Handoff in gold italic for visual contrast
        handoff_run = run(handoff.strip(), font="Inter", sz=20, italic=True, color=C["icon_gold"])
...
def render_box_key(content):
    ...
    zwsp_top = run("\u200B", font="Inter", sz=10, color=C["page_bg"])
    ...
    label_run = run(
        ...
        font="Inter", sz=16, bold=True, color=DARK_GOLD, letter_spacing=30
    )
        ...
        if "_" in text or "^" in text or "\\" in text:
            ...
            for tok in tokens:
                if not tok:
                    continue
                if tok.isspace():
                    out += run(tok, font="Inter", sz=22, italic=True, color=C["secondary"])
                elif "_" in tok or "^" in tok or "\\" in tok:
                    out += render_subscript_text(tok, font="Consolas", sz=22, color=C["secondary"], bold=False)
                else:
                    out += run(tok, font="Inter", sz=22, italic=True, color=C["secondary"], letter_spacing=10)
            return out
        return run(text, font="Inter", sz=22, italic=True, color=C["secondary"], letter_spacing=10)
    ...
    zwsp_bottom = run("\u200B", font="Inter", sz=10, color=C["page_bg"])
...
def render_box_purple(content):
    ...
    label_rpr = rpr(font="Inter", sz=16, bold=True, italic=True, color=C["icon_purple"], letter_spacing=20)
    paras.append(para(
    ...
    if cleaned:
        content_rpr = rpr(font="Inter", sz=18, italic=True, color=C["muted"])
        paras.append(para(
...
def render_data_cards(content):
    ...
    for card in cards:
        ...
        label_run = run(label, font="Inter", sz=15, color=C["muted"], small_caps=True, letter_spacing=20)
        cell_xml += para(label_run, spacing_before=0, spacing_after=40)

        # Value: 18pt bold purple
        value_run = run(value, font="Inter", sz=36, bold=True, color=C["section_purple"])
        cell_xml += para(value_run, spacing_before=0, spacing_after=40)

        # Sub: 8.5pt secondary
        if sub:
            sub_run = run(sub, font="Inter", sz=17, color=C["secondary"])
...
def render_box(box_type, content, suppress_label=False):
    ...
    if supplement_severity:
        sup_run = run("GHI CHÚ", font="Inter", sz=16, bold=True, color=C["section_purple"], letter_spacing=30)
        ...
        label_run = run(
            spec["label"],
            font="Inter", sz=16, bold=True, color=spec["accent"],
            letter_spacing=20
        )
...
def parse_markup(markup_text):
    ...
    formula_running = 0  # running counter through entire module
    example_running = 0  # running counter through entire module
    ...
    while i < len(lines):
        ...
        if line.startswith('[SECTION_OPEN]'):
            ...
            blocks.append(("SECTION_OPEN", (why_now, preview)))
            i += 1
            continue
        ...
        if m_form:
            ...
            formula_running += 1
            if f_name:
                name_map[f_name] = f"Công thức {module_num_for_numbering}.{formula_running}"
            blocks.append(("FORMULA", (formula_content.strip(), f_name)))
            i += 1
            continue
        ...
        if box_match:
            ...
            if box_type == "BOX_EXAMPLE":
                example_running += 1
                if box_name:
                    name_map[box_name] = f"Ví dụ {module_num_for_numbering}.{example_running}"
            close_tag = f'[/{box_type}]'
...
def _figure_caption_para(caption):
    ...
    cap_runs = run(caption, font=ACTIVE_FONTS.get("body", "Inter"), sz=18,
...
def render_viz_marker(data):
    ...
    line1 = run(f'\u3010H\u00ccNH {data.get("id", "?")}\u3011 ', font=ACTIVE_FONTS.get("body", "Inter"),
                sz=18, color=C["secondary"], bold=True)
    line1 += run(data.get("caption", ""), font=ACTIVE_FONTS.get("body", "Inter"), sz=18, color=C["muted"])
    line2 = run(f'ch\u00e8n \u1ea3nh: *_{data.get("id", "?")}.png (component {data.get("component", "viz")})',
                font=ACTIVE_FONTS.get("body", "Inter"), sz=16, color=C["muted"], italic=True)
...
def render_figure(path, caption, build_mode='both'):
    ...
    if not os.path.exists(path):
        ...
        ph = run('\u3010H\u00ccNH CH\u01afA RENDER\u3011 ', font=ACTIVE_FONTS.get("body", "Inter"),
                 sz=18, color=C["secondary"], bold=True)
        ph += run(caption, font=ACTIVE_FONTS.get("body", "Inter"), sz=18, color=C["muted"], italic=True)
    ...
    fig_para = para(drawing, jc="center", spacing_before=160, spacing_after=40)
    ...
    for block_type, data in blocks:
        if block_type == "COVER":
            ...
            if isinstance(data, tuple):
                body_parts.append(render_formula(data[0]))
            else:
    ...
    <w:hdr xmlns:w="http://schemas.openxmlformats.org/wordprocessingml/2006/main"
      ...
      <w:p>
        <w:pPr>
          ...
          <w:rPr><w:rFonts w:ascii="Raleway" w:hAnsi="Raleway"/><w:i/><w:sz w:val="18"/></w:rPr>
        </w:pPr>
        <w:r><w:rPr><w:rFonts w:ascii="Raleway" w:hAnsi="Raleway"/><w:i/><w:sz w:val="18"/><w:color w:val="{C['header_footer']}"/></w:rPr><w:t xml:space="preserve">CFA Level II - </w:t></w:r>
        <w:r><w:rPr><w:rFonts w:ascii="Raleway" w:hAnsi="Raleway"/><w:i/><w:sz w:val="18"/><w:color w:val="{C['section_purple']}"/></w:rPr><w:t xml:space="preserve">{esc(subject)}</w:t></w:r>
        <w:r><w:rPr><w:rFonts w:ascii="Raleway" w:hAnsi="Raleway"/><w:i/><w:sz w:val="18"/><w:color w:val="{C['header_footer']}"/></w:rPr><w:t xml:space="preserve"> - </w:t></w:r>
        <w:r><w:rPr><w:rFonts w:ascii="Raleway" w:hAnsi="Raleway"/><w:i/><w:sz w:val="18"/><w:color w:val="{C['gold']}"/></w:rPr><w:t xml:space="preserve">Module {module_num}</w:t></w:r>
        <w:r><w:rPr><w:rFonts w:ascii="Raleway" w:hAnsi="Raleway"/><w:i/><w:sz w:val="18"/><w:color w:val="{C['header_footer']}"/></w:rPr><w:t xml:space="preserve">: {esc(module_name)}</w:t></w:r>
      </w:p>
    </w:hdr>""")
...
def generate_footer():
    ...
    <w:ftr xmlns:w="http://schemas.openxmlformats.org/wordprocessingml/2006/main">
      <w:p>
        ...
        <w:r><w:rPr><w:rFonts w:ascii="Raleway" w:hAnsi="Raleway"/><w:i/><w:sz w:val="18"/><w:color w:val="{C['header_footer']}"/></w:rPr><w:t xml:space="preserve">Page </w:t></w:r>
        <w:r><w:rPr><w:rFonts w:ascii="Raleway" w:hAnsi="Raleway"/><w:i/><w:sz w:val="18"/><w:color w:val="{C['section_purple']}"/></w:rPr><w:fldChar w:fldCharType="begin"/></w:r>
        <w:r><w:rPr><w:rFonts w:ascii="Raleway" w:hAnsi="Raleway"/><w:i/><w:sz w:val="18"/><w:color w:val="{C['section_purple']}"/></w:rPr><w:instrText> PAGE </w:instrText></w:r>
        <w:r><w:rPr><w:rFonts w:ascii="Raleway" w:hAnsi="Raleway"/><w:i/><w:sz w:val="18"/><w:color w:val="{C['section_purple']}"/></w:rPr><w:fldChar w:fldCharType="end"/></w:r>
      </w:p>
    </w:ftr>""")
    ...
    <a:theme xmlns:a="http://schemas.openxmlformats.org/drawingml/2006/main" name="NoteTheme">
      <a:themeElements>
        ...
        <a:fontScheme name="Custom">
          <a:majorFont><a:latin typeface="Raleway"/><a:ea typeface=""/><a:cs typeface=""/></a:majorFont>
...
def main():
    ...
    parser.add_argument('--font-stack', choices=['A', 'B', 'C'], default='B',
                        help='Font stack: A (Editorial Source Serif), B (Modern Inter, default), C (Be Vietnam Pro)')
    ...
    # Set module number for numbering convention before parse
    ...
    if name_map:
        ...
        for bt, bd in blocks:
            if bt == "BODY" and isinstance(bd, str):
            ...
            elif bt in ("BOX_KEY", "BOX_EXAMPLE", "BOX_WARN", "BOX_NOTE", "BOX_PURPLE") and isinstance(bd, str):
                resolved = bd
                for n, label in name_map.items():
                    resolved = resolved.replace('{ref:' + n + '}', label)
                new_blocks.append((bt, resolved))
                ...
                new_blocks.append((bt, bd))
        blocks = new_blocks
    print(f"Parsed {len(blocks)} blocks; collected {len(terms_dict)} terms, {len(formulas_list)} formulas")
