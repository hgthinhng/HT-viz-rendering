---
name: opvia-content-refine
description: Editor skill chải chuốt ngôn ngữ và làm sạch văn phong cho bài phân tích/báo cáo Opvia trước khi polish thành PDF. Xử lý duy nhất NGỮ NGHĨA (Vietlish, từ vay mượn, AI-style filler, câu dài lan man, hedge overuse), không động đến CSS/HTML/visual. Hỗ trợ 3 mức (light/standard/heavy) tùy density Vietlish. LUÔN dùng skill này khi user paste text Vietlish hoặc AI-generated và muốn cleaner version, hoặc khi user nói 'refine', 'chải chuốt', 'làm sạch văn phong', 'rewrite cho ra tiếng Việt thuần', 'tinh chỉnh từ vựng', '/refine', '/opvia-refine'. Pipeline: text gốc -> [opvia-content-refine] -> text sạch -> [opvia-DailyReport-polish hoặc opvia-DeepAnalysis-polish] -> PDF. KHÔNG dùng cho text đã sạch của analyst nội bộ, text gốc tiếng Anh, hoặc khi user chỉ muốn polish visual ngay.
---

# Opvia Content Refine Skill

Skill này chải chuốt **NGỮ NGHĨA** của bài phân tích/báo cáo Opvia trước khi đem polish thành PDF. Chỉ liên quan đến từ vựng, văn phong, cách diễn đạt - KHÔNG liên quan đến CSS/HTML/render.
...
**KHÔNG dùng khi:**
- Content do người Opvia (analyst) viết - thường đã sạch
...
**Trigger:** `/refine level=1` hoặc user nói "nhẹ thôi"
...
- ✅ Áp dụng Opvia voice rules
- ✅ Vary bullet phrasing
- ✅ Vary transitions

**Trigger:** `/refine level=3` hoặc user nói "rewrite mạnh", "Opvia voice"
...
**Fix:** thay bằng marker phrase Opvia ("Đây là", "Thực ra", "Đáng chú ý là").
...
## Bước 4: Opvia voice rules (cho Level 3)
...
- Term Opvia-specific user định nghĩa → tôn trọng
...
- [ ] (Level 3) Voice Opvia áp dụng - assertion rõ, ít hedge
