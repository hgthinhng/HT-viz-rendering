---
name: opvia-DailyReport-polish
description: Polish skill biến nội dung tổng hợp phiên giao dịch hằng ngày của Opvia Advisory thành PDF chuyên nghiệp theo Opvia Design Bible. CHỈ phục vụ daily report: stat cards, bảng giá Mã/Giá/%/GTGD, mã cảnh báo khối lượng, callout cảnh báo, theme Advisory. LUÔN dùng skill này khi user paste khối văn bản tổng hợp phiên giao dịch (có 'phiên', 'VN-Index', section đánh số 01/02/03, bảng cổ phiếu Mã/Giá/%) và muốn xuất PDF, hoặc khi user nói 'polish daily', 'publish daily', '/opvia-daily', 'làm pdf báo cáo phiên', 'xuất pdf daily Opvia'. KHÔNG dùng cho bài phân tích sâu/xã luận/deep dive nhiều Phần - dùng opvia-DeepAnalysis-polish thay vào đó.
---

# Opvia Daily Report Polish Skill

Biến text tổng hợp phiên giao dịch hằng ngày của Opvia Advisory thành PDF chuyên nghiệp theo chuẩn Opvia Design Bible. Skill này CHỈ phục vụ daily report (information density: stat cards, bảng giá, mã cảnh báo). Cho bài phân tích sâu/xã luận dùng `opvia-DeepAnalysis-polish`.
...
3. Heading dùng Playfair Display (serif). Số liệu dùng JetBrains Mono. Body dùng Inter.
4. Bảng màu: Prussian `#003153`, Brass `#B5A642`, Slate `#4A6FA5`, Ivory `#F5F1E8`, Charcoal `#2B2B2B`. Bordeaux `#722F37` chỉ cho rủi ro nghiêm trọng.
...
```python
import sys
SKILL_DIR = "/path/to/opvia-DailyReport-polish"  # path thực tế khi run
...
### 3c. Cấp độ callout
- `.callout.key`: Thông điệp chính (Prussian background, ivory text)
- `.callout.warn`: Cảnh báo rủi ro, "cắt lỗ" (Bordeaux border)
- `.callout`: Default, ghi chú thêm (Brass border)
...
<div class="cover-daily">
  ...
  <div class="brand-row">OPVIA ADVISORY</div>
...
<div class="content">
  ...
  <div class="disclaimer-card">
    ...
    <p>OPVIA Advisory biên soạn cho ngày DD/MM/YYYY... <strong>không phải khuyến nghị đầu tư</strong>...</p>
  </div>
</div>
```
...
```python
import sys
SKILL_DIR = "/path/to/opvia-DailyReport-polish"
sys.path.insert(0, SKILL_DIR)
...
render_pdf(html, "/mnt/user-data/outputs/OPVIA_DAILY_20260423.pdf")
```

**Naming convention:** `OPVIA_DAILY_YYYYMMDD.pdf`
...
```
/mnt/user-data/outputs/OPVIA_DAILY_YYYYMMDD.pdf
```
