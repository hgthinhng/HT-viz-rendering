"""
Opvia Data Viz - Validators + Ticker matcher
=============================================
...
PALETTE_LOCKED = {
    "#003153": "Prussian",
    "#000d18": "Prussian-900 (cover dark)",
    "#B5A642": "Brass",
    "#722F37": "Bordeaux",
    "#4A6FA5": "Slate",
    "#F5F1E8": "Ivory",
...
PALETTE_ALLOWED_TINTS = {
    "#fdfcf8", "#f0ece0", "#e8e2d5", "#dcd5c5",   # paper variants
    "#1a3556", "#2d4a6f",                           # prussian shades
    "#c9b96f", "#8a7a2e",                           # brass shades
    "#a3434a", "#5a242a",                           # bordeaux shades
}
PALETTE_ALLOWED_LOWER = (
    set(PALETTE_LOCKED_LOWER.keys()) | PALETTE_ALLOWED_TINTS
)
...
def validate_palette(html: str) -> list[str]:
    ...
    for m in pattern.finditer(html):
        ...
        if hex_val in seen:
            continue
        seen.add(hex_val)
        if hex_val not in PALETTE_ALLOWED_LOWER:
            # Allow #fff/#000 nhưng warn (Opvia rule: paper/charcoal thay)
            if hex_val in ("#ffffff", "#000000"):
                errors.append(
                    f"Pure {hex_val} dùng trực tiếp - thay #FAF7F0 (paper) "
                    f"hoặc #2B2B2B (charcoal)."
                )
            else:
                errors.append(f"Hex color {hex_val} ngoài palette Opvia.")
    return errors


def validate_vn_numbers(html: str) -> list[str]:
    """Warn nếu số dùng dấu chấm thập phân thay vì dấu phẩy.
...
def validate_font_whitelist(html: str) -> list[str]:
    """Warn nếu CSS font-family ngoài 3 stacks Opvia."""
    warnings = []
...
if __name__ == "__main__":
    ...
    assert validate_palette('color:#003153;') == []
