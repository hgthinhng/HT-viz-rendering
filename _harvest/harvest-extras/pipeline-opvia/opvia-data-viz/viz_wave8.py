...
7. compass_callout        - Wrapper bọc viz với brass leader line + sidebar caption
...
def line_with_annotations(
    ...
    series: list of dicts, mỗi dict 1 line:
        {
            "name": "DXY",
            "color": "#003153",      # optional, default Prussian
    ...
    for s_idx, s in enumerate(series):
        color = s.get("color", COLORS.PRUSSIAN if s_idx == 0 else COLORS.SLATE)
    ...
    for ann in annotations:
        ...
        # marker dot
        ann_html.append(
            f'<circle cx="{ax:.1f}" cy="{ay:.1f}" r="3.5" fill="{COLORS.BRASS}" '
            f'stroke="{COLORS.IVORY}" stroke-width="1.5"/>'
        )
        # leader line
        ann_html.append(
            f'<line x1="{ax:.1f}" y1="{ay:.1f}" x2="{line_x2:.1f}" y2="{line_y2:.1f}" '
            f'stroke="{COLORS.BRASS}" stroke-width="0.7" stroke-dasharray="1.5,1.5"/>'
        )
        # value label (mono, larger)
        if value_label:
            ann_html.append(
                f'<text x="{tx:.1f}" y="{ty:.1f}" text-anchor="{text_anchor}" '
                f'font-family="{FONTS.MONO}" font-size="8.5" fill="{COLORS.PRUSSIAN}" font-weight="600">{escape(value_label)}</text>'
            )
            ty_text = ty + 9
        else:
            ty_text = ty
...
def dot_plot_distribution(
    ...
    if show_quartiles:
        ...
        quartile_html = (
            ...
            f'fill="{COLORS.BRASS_50}" stroke="{COLORS.BRASS_300}" stroke-width="0.6" rx="2"/>'
        )
    ...
    if show_median:
        ...
        median_html = (
            ...
            f'stroke="{COLORS.PRUSSIAN}" stroke-width="1.6"/>'
            f'<text x="{x_m:.1f}" y="{cy - 26}" text-anchor="middle" '
            f'font-family="{FONTS.MONO}" font-size="7" fill="{COLORS.PRUSSIAN}" font-weight="600">'
    ...
    for it in items:
        ...
        color = COLORS.BORDEAUX if it.get("danger") else (
            COLORS.BRASS if is_hl else COLORS.PRUSSIAN
        )
...
def small_multiples_grid(
    ...
    for idx, p in enumerate(panels):
        ...
        v_color = {
            ...
            "neutral": COLORS.PRUSSIAN,
        }.get(v_color_key, COLORS.PRUSSIAN)
        ...
        <g>
          ...
          <text x="{ox + 8}" y="{oy + 12}" font-family="{FONTS.SANS}" font-size="7.8"
                font-weight="700" fill="{COLORS.PRUSSIAN}" letter-spacing="0.04em">{panel_label}</text>
...
def quadrant_scatter(
    ...
    points: [{"label": "VCB", "x": 1.2, "y": 19.2, "color": "#003153", "size": 1.0}]
    ...
    if quadrants.get("tr"):
        quad_html.append(_multiline_text(
            W - PAD_R - 8, PAD_T + 14, quadrants["tr"], "end", COLORS.BRASS, 9.5, 0.7, "top"
        ))
    if quadrants.get("bl"):
        quad_html.append(_multiline_text(
    ...
    for p in points:
        ...
        pts_html.append(
            ...
            f'font-family="{FONTS.MONO}" font-size="7.5" fill="{COLORS.PRUSSIAN}" font-weight="700">{lbl}</text>'
        )
...
def slopegraph(
    ...
    header_html = (
        ...
        f'font-family="{FONTS.SANS}" font-size="8.5" font-weight="700" fill="{COLORS.PRUSSIAN}" '
    ...
    for it in sorted_items:
        ...
        delta = a - b
        if is_hl:
            color = COLORS.BRASS if delta > 0 else COLORS.BORDEAUX
        elif delta > 0:
        ...
        lbl = escape(it["label"])
        weight = "700" if is_hl else "500"
        ink = COLORS.PRUSSIAN if is_hl else COLORS.CHARCOAL
        labels_left_html.append(
...
def sankey_mini(
    ...
    sources / targets: [{"id": "ng_hang", "label": "Ngân hàng", "color": "#003153"}]
    ...
    for s in sources:
        ...
        src_layout[s["id"]] = {"y": src_y, "h": h, "value": v, "label": s.get("label", s["id"]),
                                "color": s.get("color", COLORS.PRUSSIAN)}
        src_y += h + gap
    ...
    header_html = (
        ...
        f'font-family="{FONTS.SANS}" font-size="8" font-weight="700" fill="{COLORS.PRUSSIAN}" '
...
def compass_callout(
    ...
    - Brass dot ở target point
    - Leader line cong nhẹ (Bezier) chạy ra sidebar
    - Italic caption ở sidebar với vertical brass rule
