# Catalog Index - StockLPT Data Viz

33 component spread Wave 1-10 + 2 page templates. **Skim trang này trước khi deep-read spec cụ thể.**

> **Brand note:** mọi hex trong các spec dưới là **source space của StockLPT**. Khi brand active là `stocklpt`, engine `_brand.py` remap palette + wordmark sang tím–vàng tại bước `render_pdf` cuối — không cần đổi spec hay truyền brand vào component. Xem mục Bảng màu trong `SKILL.md`.
...
|---|---|---|
| `cover_deep_page` | Trang bìa Indigo-900 với hero stat + 3 takeaways | Mọi deep analysis (trừ daily report) |
| `back_cover_cta` | Trang kết với CTA Indigo box + nguồn + disclaimer | Mọi bài publish-ready (deep + daily) |

Mở bài bằng `cover_deep_page`, kết bằng `back_cover_cta`. Đối xứng visual signature của StockLPT.
...
| `comparison_cards` | Cards so sánh side-by-side | Phương án A vs B (rất StockLPT signature) |
...
| `capsule_verdict` | Trang hero magazine 1 verdict + mega-number duotone | Mở/chốt deep dive doanh nghiệp với 1 con số rủi ro signature (full-page) |
| `scenario_outlook` | Bull/base/bear cổ phiếu, editorial phẳng (trigger/implication) | Kịch bản định giá cổ phiếu — thay `scenario_cards` cho equity |
...
| Component | One-liner | Best for |
|---|---|---|
| `marginalia` | Chú thích lề accent italic 75/22 | Methodology notes, definition tooltips, source |
...
| `infographic_panel` | Dark Indigo panel hero 56pt + 4 facts | Cover signature, section opener art |
...
## Wave 12 (chart định lượng matplotlib → PNG) - `viz_charts`

> Chart dữ liệu THẬT (time-series, ranking, diverging, waterfall, scatter) mà atom inline
> không vẽ tốt. Tự theme theo brand (up=emerald, down=đỏ, chrome=forest accent). Spec đầy đủ:
> `catalog/viz_charts.md`.

| Component | One-liner | Best for |
|---|---|---|
| `chart_line` | Line/time-series + marker + ref line | Đường VN-Index trong phiên, xu hướng giá/chỉ số |
| `chart_diverging` | Diverging bar +/- (emerald/đỏ) | Dòng tiền ròng mua/bán theo mã |
| `chart_bar_h` | Ranking ngang (highlight tùy chọn) | Top GTGD, đóng góp điểm chỉ số |
| `chart_waterfall` | Waterfall phân rã có tính toán | P&L bridge, walk vốn |
| `chart_bar_grouped` | Grouped bar nhiều kỳ/nhóm | So sánh kỳ này vs kỳ trước theo nhóm |
| `chart_scatter` | Scatter + quadrant tùy chọn | Tương quan P/B × ROE, định vị nhóm |
