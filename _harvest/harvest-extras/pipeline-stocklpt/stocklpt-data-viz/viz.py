"""
StockLPT Data Viz - 6 components cho bài phân tích sâu tài chính
...
SECTOR_COLORS = {
    # Banks
    "big4_socb": "#2A1A4A",      # indigo
    "tier1_jsb": "#514B78",      # slate
    "tier2_jsb": "#16633C",      # accent
    "tier3_jsb": "#645B76",      # gray
    "foreign_bank": "#7A1F35",   # bordeaux
...
def color_for_ticker(ticker: str) -> str:
    ...
    Examples:
        color_for_ticker("VCB") -> "#2A1A4A"
        color_for_ticker("VPB") -> "#514B78"
        color_for_ticker("UNKNOWN") -> "#514B78" (fallback)
    """
...
def viz_styles() -> str:
    ...
    return r"""
/* ============================================================
   STOCKLPT DATA VIZ - components stylesheet
   ============================================================ */
...
.viz-bar-title {
  ...
  font-size: 10pt; font-weight: 700; color: #2A1A4A;
  margin-bottom: 2mm; padding-bottom: 3px;
  border-bottom: 0.6px solid rgba(81,75,120,0.18);
}
.viz-bar-subtitle {
  font-family: 'Inter', 'InterVN', sans-serif;
  font-size: 8.5pt; font-weight: 400; color: #514B78;
  margin-bottom: 4mm;
}
.viz-bar-row {
  display: grid; grid-template-columns: 22mm 1fr 18mm;
  gap: 8px; align-items: center;
  padding: 5px 0; border-bottom: 0.4px solid rgba(81,75,120,0.08);
}
.viz-bar-label {
  font-family: 'Inter', 'InterVN', sans-serif;
  font-size: 9pt; font-weight: 600; color: #2A1A4A;
}
.viz-bar-note {
  font-family: 'Inter', 'InterVN', sans-serif;
...
.viz-bar-track {
  ...
  background: rgba(81,75,120,0.08); border-radius: 2px;
}
.viz-bar-fill {
  height: 100%; border-radius: 2px;
  background: #2A1A4A;
}
.viz-bar-fill.danger { background: #E13453; }
.viz-bar-fill.warn { background: #16633C; }
.viz-bar-fill.positive { background: #21B36A; }
.viz-bar-threshold {
  position: absolute; top: -3px; bottom: -3px;
  width: 1.5px; background: #7A1F35;
}
.viz-bar-threshold-label {
  position: absolute; top: -16px; transform: translateX(-50%);
  font-family: 'JBM', 'JBMVN', monospace;
  font-size: 7pt; font-weight: 600; color: #7A1F35;
...
.viz-bar-value.danger { color: #E13453; font-weight: 700; }
...
.viz-gauge-bg-arc { fill: none; stroke: #DEE4EC; stroke-width: 14; stroke-linecap: round; }
.viz-gauge-fill-arc { fill: none; stroke-width: 14; stroke-linecap: round; }
.viz-gauge-fill-arc.normal { stroke: #16633C; }
.viz-gauge-fill-arc.danger { stroke: #E13453; }
.viz-gauge-threshold-tick { stroke: #7A1F35; stroke-width: 2.5; }
.viz-gauge-value-text {
  font-family: 'JBM', 'JBMVN', monospace; font-weight: 700; fill: #2A1A4A;
}
.viz-gauge-label-text {
  font-family: 'Inter', 'InterVN', sans-serif; font-weight: 700; fill: #645B76;
  letter-spacing: 0.14em; text-transform: uppercase;
}
.viz-gauge-threshold-text {
  font-family: 'JBM', 'JBMVN', monospace; font-weight: 600; fill: #7A1F35;
  letter-spacing: 0.06em;
}
.viz-gauge-desc {
  font-family: 'Inter', 'InterVN', sans-serif;
  font-size: 9pt; line-height: 1.4; color: #221A34;
...
.viz-heatmap-subtitle {
  font-family: 'Inter', 'InterVN', sans-serif;
  font-size: 8.5pt; color: #514B78; margin-bottom: 4mm;
}
.viz-heatmap-grid {
  display: grid; gap: 2px;
  border: 0.6px solid rgba(81,75,120,0.15);
  background: rgba(81,75,120,0.10);
}
.viz-heatmap-cell {
...
.viz-heatmap-row-header {
  background: #F4F6F9; font-weight: 700; color: #2A1A4A;
...
.viz-heatmap-cell .cell-note {
  font-size: 7pt; color: #645B76;
  margin-top: 2px;
}
.viz-heatmap-cell.pos-strongest {
  background: #21B36A; color: #FFFFFF;
}
.viz-heatmap-cell.pos-strong {
  background: rgba(33,179,106,0.55); color: #2A1A4A;
}
.viz-heatmap-cell.pos {
  background: rgba(33,179,106,0.22); color: #2A1A4A;
}
.viz-heatmap-cell.neutral {
  background: rgba(81,75,120,0.08); color: #514B78;
}
.viz-heatmap-cell.neg {
  background: rgba(225,52,83,0.18); color: #7A1F35;
}
.viz-heatmap-cell.neg-strong {
  background: rgba(225,52,83,0.45); color: #7A1F35;
}
.viz-heatmap-cell.neg-strongest {
  background: #E13453; color: #FFFFFF;
}
...
.viz-flow-node-rect.cause { fill: #2A1A4A; }
.viz-flow-node-rect.intermediate { fill: #F4F6F9; stroke: #514B78; stroke-width: 0.8; }
.viz-flow-node-rect.result { fill: #16633C; }
.viz-flow-node-text {
  font-family: 'Inter', 'InterVN', sans-serif;
  font-size: 8.5pt; font-weight: 600;
}
.viz-flow-node-text.light { fill: #F4F6F9; }
.viz-flow-node-text.dark { fill: #2A1A4A; }
.viz-flow-arrow { stroke: #514B78; stroke-width: 1.2; fill: none; }
.viz-flow-arrowhead { fill: #514B78; }
...
.viz-scenario {
  background: #F4F6F9; border-top: 3px solid #16633C;
  padding: 5mm 5mm 5mm; border-radius: 2px;
  page-break-inside: avoid;
  display: flex; flex-direction: column;
}
.viz-scenario.base { border-top-color: #514B78; }
.viz-scenario.positive { border-top-color: #21B36A; }
.viz-scenario.negative { border-top-color: #E13453; }
.viz-scenario-header {
...
.viz-scenario-name {
  ...
  font-size: 12pt; font-weight: 700; color: #2A1A4A;
}
.viz-scenario-prob {
  font-family: 'JBM', 'JBMVN', monospace;
  font-size: 11pt; font-weight: 700; color: #16633C;
}
.viz-scenario-row {
  margin-bottom: 2.5mm;
}
...
.viz-scenario-label {
  ...
  text-transform: uppercase; color: #645B76;
  margin-bottom: 1px;
}
.viz-scenario-value {
  font-family: 'Inter', 'InterVN', sans-serif;
  font-size: 8.5pt; line-height: 1.45; color: #221A34;
}
.viz-scenario-value.winners { color: #21B36A; font-weight: 600; }
.viz-scenario-value.losers { color: #E13453; font-weight: 600; }
...
.viz-timeline-axis { stroke: #514B78; stroke-width: 0.8; }
.viz-timeline-tick { stroke: #514B78; stroke-width: 0.8; }
.viz-timeline-dot.milestone { fill: #514B78; }
.viz-timeline-dot.decision { fill: #16633C; }
.viz-timeline-dot.result { fill: #21B36A; }
.viz-timeline-dot.current { fill: #E13453; }
.viz-timeline-date {
  font-family: 'JBM', 'JBMVN', monospace;
  font-size: 7pt; font-weight: 600; fill: #2A1A4A;
}
.viz-timeline-label {
  font-family: 'Inter', 'InterVN', sans-serif;
  font-size: 8pt; fill: #221A34;
}
...
def gauge(
    ...
    Args:
        ...
        danger_above: if value > this, fill color = red instead of accent
    ...
    fill_color = "#E13453" if is_danger else "#16633C"
    ...
    if threshold is not None and threshold <= max_val:
        ...
        threshold_html = f'<line x1="{outer_x:.2f}" y1="{outer_y:.2f}" x2="{inner_x:.2f}" y2="{inner_y:.2f}" stroke="#7A1F35" stroke-width="2.5"/>'
        ...
        threshold_text_html = f'<text x="{out_x:.2f}" y="{out_y:.2f}" text-anchor="{anchor}" font-family="JBM,JBMVN,monospace" font-size="7" font-weight="600" fill="#7A1F35">Trần {thr_label}</text>'
    ...
    if annotations:
        for ann in annotations:
            ...
            annotations_html += f"""
            <line {line_attrs} stroke="#7A1F35" stroke-width="0.6" stroke-dasharray="1.5,1"/>
            <text x="{tx:.1f}" y="{ty:.1f}" text-anchor="{anchor}" font-family="Inter,InterVN,sans-serif" font-size="6.5" font-weight="600" fill="#7A1F35">{escape(ann_text)}</text>
            """
...
<div class="viz-gauge">
  <svg class="viz-gauge-svg" viewBox="0 0 240 130" xmlns="http://www.w3.org/2000/svg">
    ...
    <path d="{bg_path}" fill="none" stroke="#DEE4EC" stroke-width="14" stroke-linecap="round"/>
    <path d="{fg_path}" fill="none" stroke="{fill_color}" stroke-width="14" stroke-linecap="round"/>
    {threshold_html}
    {threshold_text_html}
    {annotations_html}
    <text x="{cx}" y="92" text-anchor="middle" font-family="JBM,JBMVN,monospace" font-size="22" font-weight="700" fill="#2A1A4A">{val_str}</text>
    <text x="{cx}" y="115" text-anchor="middle" font-family="Inter,InterVN,sans-serif" font-size="6.5" font-weight="700" fill="#645B76" letter-spacing="0.5">{escape(label)}</text>
    </g>
  </svg>
  {desc_html}
</div>
...
def flow_bridge(nodes: list[dict], title: str = "") -> str:
    ...
    for i, node in enumerate(nodes):
        ...
        text = node.get("text", "")
        y = 10 + i * (node_h + gap)
        text_color = "#F4F6F9" if node_type == "cause" else "#2A1A4A"
        bg = type_colors.get(node_type, "#F4F6F9")
        ...
        if i < n - 1:
            ...
            ax = cx + node_w / 2
            elems.append(f"""
            <line x1="{ax}" y1="{arrow_y_start}" x2="{ax}" y2="{arrow_y_end}" stroke="#514B78" stroke-width="1.2"/>
            <polygon points="{ax-3},{arrow_y_end-1} {ax+3},{arrow_y_end-1} {ax},{arrow_y_end+4}" fill="#514B78"/>
            """)
...
def scenario_cards(scenarios: list[dict]) -> str:
    ...
    for s in scenarios:
        ...
        winners = escape(_listjoin(s.get("winners", "")))
        losers = escape(_listjoin(s.get("losers", "")))
...
def timeline_horizontal(events: list[dict], title: str = "") -> str:
    ...
    elems = [
        f'<line x1="{margin_x}" y1="{axis_y}" x2="{svg_w - margin_x}" y2="{axis_y}" stroke="#514B78" stroke-width="0.8"/>'
    ]
    ...
    for i, ev in enumerate(events):
        ...
        date = ev.get("date", "")
        label = ev.get("label", "")
        dot_color = type_dot_colors.get(ev_type, "#514B78")
        ...
        if len(label) > max_chars:
            ...
            label_svg = f"""
            <text x="{x}" y="{axis_y + 22}" text-anchor="middle" font-family="Inter,InterVN,sans-serif" font-size="8" fill="#221A34">{escape(line1)}</text>
            <text x="{x}" y="{axis_y + 32}" text-anchor="middle" font-family="Inter,InterVN,sans-serif" font-size="8" fill="#221A34">{escape(line2)}</text>
            """
        else:
            label_svg = f'<text x="{x}" y="{axis_y + 22}" text-anchor="middle" font-family="Inter,InterVN,sans-serif" font-size="8" fill="#221A34">{escape(label)}</text>'

        # Tick line and dot
        elems.append(f'<line x1="{x}" y1="{axis_y - 4}" x2="{x}" y2="{axis_y + 4}" stroke="#514B78" stroke-width="0.8"/>')
        elems.append(f'<circle cx="{x}" cy="{axis_y}" r="5" fill="{dot_color}"/>')
        # Date above
        elems.append(f'<text x="{x}" y="{axis_y - 10}" text-anchor="middle" font-family="JBM,JBMVN,monospace" font-size="7" font-weight="600" fill="#2A1A4A">{escape(date)}</text>')
        # Label below
...
.viz-ranking-row {
  ...
  gap: 8px; align-items: center;
  padding: 6px 0; border-bottom: 0.4px solid rgba(81,75,120,0.10);
}
.viz-ranking-rank {
  font-family: 'JBM', 'JBMVN', monospace;
  font-size: 11pt; font-weight: 700; color: #16633C;
...
.viz-ranking-label {
  font-family: 'Inter', 'InterVN', sans-serif;
  font-size: 9pt; font-weight: 700; color: #2A1A4A;
}
.viz-ranking-value {
  font-family: 'JBM', 'JBMVN', monospace;
  font-size: 9pt; font-weight: 600; color: #2A1A4A;
...
.viz-ranking-delta.up { color: #21B36A; }
.viz-ranking-delta.down { color: #E13453; }
.viz-ranking-delta.flat { color: #645B76; }
.viz-ranking-delta.new { color: #16633C; font-style: italic; }
...
.viz-comparison-card {
  background: #F4F6F9; padding: 5mm;
  border-top: 3px solid #514B78;
  border-radius: 2px;
  page-break-inside: avoid;
}
.viz-comparison-card.fork-a { border-top-color: #16633C; }
.viz-comparison-card.fork-b { border-top-color: #514B78; }
.viz-comparison-card.positive { border-top-color: #21B36A; }
.viz-comparison-card.negative { border-top-color: #E13453; }
.viz-comparison-card.neutral { border-top-color: #645B76; }
...
.viz-comparison-metrics {
  ...
  border-bottom: 0.5px solid rgba(81,75,120,0.15);
}
...
.viz-comparison-summary {
  ...
  color: #221A34; font-style: italic;
}
...
.viz-dashboard-stat-label {
  ...
  letter-spacing: 0.14em; text-transform: uppercase;
  color: #645B76; margin-bottom: 2mm;
}
.viz-dashboard-stat-value {
  font-family: 'JBM', 'JBMVN', monospace;
  font-size: 17pt; font-weight: 700; color: #2A1A4A;
...
.viz-dashboard-stat-delta.up { color: #21B36A; }
.viz-dashboard-stat-delta.down { color: #E13453; }
.viz-dashboard-stat-delta.flat { color: #645B76; }
.viz-dashboard-stat-unit {
  font-family: 'Inter', 'InterVN', sans-serif;
  font-size: 7.5pt; color: #645B76;
  margin-top: 2px;
}
...
def waterfall(
    ...
    elems.append(f'<line x1="{margin_left}" y1="{y0_pos}" x2="{svg_w - margin_right}" y2="{y0_pos}" stroke="#514B78" stroke-width="0.6" stroke-opacity="0.4"/>')
    ...
    for i, bar in enumerate(bar_data):
        ...
        color = type_colors.get(bar["type"], "#514B78")
        ...
        elems.append(f'<text x="{x + bar_w/2:.1f}" y="{y - 4:.1f}" text-anchor="middle" font-family="JBM,JBMVN,monospace" font-size="8" font-weight="600" fill="#2A1A4A">{val_label}</text>')

        # Label dưới đáy
        label = bar["label"]
        if len(label) > 18:
            l1, l2 = smart_wrap(label, 18)
            elems.append(f'<text x="{x + bar_w/2:.1f}" y="{svg_h - margin_bottom + 14:.1f}" text-anchor="middle" font-family="Inter,InterVN,sans-serif" font-size="7.5" fill="#221A34">{escape(l1)}</text>')
            elems.append(f'<text x="{x + bar_w/2:.1f}" y="{svg_h - margin_bottom + 24:.1f}" text-anchor="middle" font-family="Inter,InterVN,sans-serif" font-size="7.5" fill="#221A34">{escape(l2)}</text>')
        else:
            elems.append(f'<text x="{x + bar_w/2:.1f}" y="{svg_h - margin_bottom + 14:.1f}" text-anchor="middle" font-family="Inter,InterVN,sans-serif" font-size="7.5" fill="#221A34">{escape(label)}</text>')
        ...
        if i < n - 1:
            ...
            if next_bar["type"] == "negative":
                ...
                connector_y_pos = margin_top + (y_max - bar["top"]) / y_range * chart_h
            elems.append(f'<line x1="{x_end_current:.1f}" y1="{connector_y_pos:.1f}" x2="{x_start_next:.1f}" y2="{connector_y_pos:.1f}" stroke="#645B76" stroke-width="0.6" stroke-dasharray="2,2"/>')
...
def distribution_dot_plot(
    ...
    # X-axis line
    elems.append(f'<line x1="{margin_left}" y1="{axis_y}" x2="{svg_w - margin_right}" y2="{axis_y}" stroke="#514B78" stroke-width="0.8"/>')
    ...
    for i in range(n_ticks):
        ...
        elems.append(f'<line x1="{tick_x:.1f}" y1="{axis_y - 3}" x2="{tick_x:.1f}" y2="{axis_y + 3}" stroke="#514B78" stroke-width="0.6"/>')
        tick_label = f"{tick_val:.0f}".replace(".", ",")
        elems.append(f'<text x="{tick_x:.1f}" y="{axis_y + 14}" text-anchor="middle" font-family="JBM,JBMVN,monospace" font-size="7" fill="#645B76">{tick_label}</text>')

    # X-axis label
    if x_label:
        elems.append(f'<text x="{svg_w/2}" y="{svg_h - 5}" text-anchor="middle" font-family="Inter,InterVN,sans-serif" font-size="8" font-weight="600" fill="#2A1A4A">{escape(x_label)}</text>')
    ...
    if annotations:
        for ann in annotations:
            ...
            elems.append(f'<line x1="{ann_x:.1f}" y1="{margin_top}" x2="{ann_x:.1f}" y2="{axis_y}" stroke="#7A1F35" stroke-width="0.6" stroke-dasharray="2,2" stroke-opacity="0.5"/>')
            elems.append(f'<text x="{ann_x:.1f}" y="{margin_top - 6}" text-anchor="middle" font-family="Inter,InterVN,sans-serif" font-size="7" font-style="italic" fill="#7A1F35">{escape(ann_text)}</text>')
    ...
    for it in sorted_items:
        ...
        fill = "#16633C" if it.get("highlight", False) else "#514B78"
        elems.append(f'<circle cx="{ax:.1f}" cy="{ay:.1f}" r="{radius}" fill="{fill}" stroke="#F4F6F9" stroke-width="0.6"/>')

        # Label cho highlighted items
        if it.get("highlight", False):
            elems.append(f'<text x="{ax:.1f}" y="{ay - 8}" text-anchor="middle" font-family="Inter,InterVN,sans-serif" font-size="7.5" font-weight="700" fill="#2A1A4A">{escape(it["label"])}</text>')
...
.viz-policy-fork-title {
  font-family: 'PFD', 'PFDVN', serif;
  font-size: 14pt; font-weight: 700; color: #2A1A4A;
  margin-bottom: 2mm;
}
.viz-policy-fork-context {
  font-family: 'PFD', 'PFDVN', serif;
  font-size: 9.5pt; font-weight: 450; color: #514B78;
  font-style: italic;
...
.viz-policy-fork-option.fork-a { border-top-color: #16633C; }
.viz-policy-fork-option.fork-b { border-top-color: #514B78; }
...
.viz-policy-fork-option.fork-b .viz-policy-fork-label { color: #514B78; }
...
.viz-policy-fork-winners {
  font-family: 'Inter', 'InterVN', sans-serif;
  font-size: 8.5pt; font-weight: 600;
  color: #21B36A; margin-bottom: 3mm;
}
.viz-policy-fork-summary {
  font-family: 'PFD', 'PFDVN', serif;
  font-size: 9pt; font-weight: 450;
  font-style: italic; color: #221A34;
  line-height: 1.5;
}
...
.viz-policy-fork-decision-label {
  ...
  color: #645B76; margin-bottom: 4mm;
}
...
.viz-policy-fork-factor-header-col.fork-a { color: #16633C; }
.viz-policy-fork-factor-header-col.fork-b { color: #514B78; }
...
.viz-policy-fork-factor-mark.win-a { color: #16633C; }
.viz-policy-fork-factor-mark.win-b { color: #514B78; }
.viz-policy-fork-factor-mark.lose { color: rgba(81,75,120,0.18); }
...
.viz-wl-summary {
  background: #2A1A4A; color: #F4F6F9;
  padding: 4mm 6mm;
  margin-bottom: 5mm;
...
.viz-wl-summary-value {
  font-family: 'JBM', 'JBMVN', monospace;
  font-size: 18pt; font-weight: 700; color: #16633C;
  line-height: 1;
}
.viz-wl-summary-label {
  font-family: 'PFD', 'PFDVN', serif;
  font-size: 9pt; font-weight: 450;
  font-style: italic; color: rgba(244,246,249,0.85);
  line-height: 1.4;
}
.viz-wl-grid {
  display: grid; grid-template-columns: 1fr 1fr;
...
.viz-wl-column.winners { border-top-color: #21B36A; }
.viz-wl-column.losers { border-top-color: #E13453; }
.viz-wl-col-label {
  font-family: 'Inter', 'InterVN', sans-serif;
...
.viz-wl-column.winners .viz-wl-col-label { color: #21B36A; }
.viz-wl-column.losers .viz-wl-col-label { color: #E13453; }
.viz-wl-col-summary {
  font-family: 'PFD', 'PFDVN', serif;
  font-size: 9pt; font-weight: 500;
  font-style: italic; color: #221A34;
  margin-bottom: 4mm; padding-bottom: 3mm;
  border-bottom: 0.5px solid rgba(81,75,120,0.18);
}
.viz-wl-item {
  padding: 3mm 0;
  border-bottom: 0.4px solid rgba(81,75,120,0.10);
...
.viz-wl-column.winners .viz-wl-item-value { color: #21B36A; }
.viz-wl-column.losers .viz-wl-item-value { color: #E13453; }
.viz-wl-item-detail {
  font-family: 'PFD', 'PFDVN', serif;
...
.viz-hero-eyebrow {
  ...
  color: #16633C; margin-bottom: 6mm;
}
.viz-hero-mega {
  font-family: 'JBM', 'JBMVN', monospace;
...
.viz-hero-unit {
  ...
  letter-spacing: 0.06em;
  color: #514B78; margin-bottom: 4mm;
}
.viz-hero-label {
  font-family: 'PFD', 'PFDVN', serif;
  font-size: 12pt; font-weight: 500;
  font-style: italic;
  color: #2A1A4A; margin-bottom: 8mm;
  line-height: 1.4;
...
.viz-hero-sub-grid {
  ...
  padding-top: 5mm;
  border-top: 0.5px solid rgba(81,75,120,0.18);
}
.viz-hero-sub-stat {
  text-align: center;
}
...
.viz-mech-title {
  font-family: 'PFD', 'PFDVN', serif;
  font-size: 13pt; font-weight: 700; color: #2A1A4A;
  margin-bottom: 4mm;
}
.viz-mech-stages {
  display: grid;
...
.viz-mech-stage.input { border-top-color: #514B78; }
.viz-mech-stage.process { border-top-color: #16633C; }
.viz-mech-stage.output { border-top-color: #21B36A; }
.viz-mech-arrow {
  display: flex; align-items: center;
...
.viz-mech-stage.input .viz-mech-stage-label { color: #514B78; }
.viz-mech-stage.process .viz-mech-stage-label { color: #16633C; }
.viz-mech-stage.output .viz-mech-stage-label { color: #21B36A; }
.viz-mech-stage-name {
  font-family: 'PFD', 'PFDVN', serif;
  font-size: 11pt; font-weight: 700;
  color: #2A1A4A; margin-bottom: 2mm;
  line-height: 1.25;
}
...
.viz-ba-state.before { border-top-color: #645B76; opacity: 0.92; }
.viz-ba-state.after { border-top-color: #21B36A; }
.viz-ba-state-label {
  font-family: 'Inter', 'InterVN', sans-serif;
  font-size: 7.5pt; font-weight: 700;
  letter-spacing: 0.16em; text-transform: uppercase;
...
.viz-ba-state.before .viz-ba-state-label { color: #645B76; }
.viz-ba-state.after .viz-ba-state-label { color: #21B36A; }
.viz-ba-state-name {
  font-family: 'PFD', 'PFDVN', serif;
...
.viz-exec-title {
  ...
  margin-bottom: 3mm;
  padding-bottom: 2mm;
  border-bottom: 1px solid #16633C;
}
.viz-exec-takeaway {
  background: #2A1A4A; color: #F4F6F9;
  padding: 6mm 7mm;
  margin-bottom: 4mm;
  border-left: 4px solid #16633C;
}
...
.viz-exec-cell.findings { border-top-color: #21B36A; }
.viz-exec-cell.risks { border-top-color: #E13453; }
.viz-exec-cell.watch { border-top-color: #514B78; }
.viz-exec-cell-label {
  font-family: 'Inter', 'InterVN', sans-serif;
  font-size: 7pt; font-weight: 700;
  letter-spacing: 0.14em; text-transform: uppercase;
...
.viz-exec-cell.findings .viz-exec-cell-label { color: #21B36A; }
.viz-exec-cell.risks .viz-exec-cell-label { color: #E13453; }
.viz-exec-cell.watch .viz-exec-cell-label { color: #514B78; }
.viz-exec-bullet {
...
.viz-exec-cell.findings .viz-exec-bullet-mark { color: #21B36A; }
.viz-exec-cell.risks .viz-exec-bullet-mark { color: #E13453; }
.viz-exec-cell.watch .viz-exec-bullet-mark { color: #514B78; }
...
def scenario_matrix(
    ...
    quadrants = [
        ...
        (margin["left"], margin["top"], chart_w/2, chart_h/2, "#F4F6F9", 0.5),  # top-left
        (margin["left"] + chart_w/2, margin["top"], chart_w/2, chart_h/2, "#F0EAE0", 0.7),  # top-right (high impact + high prob = critical)
        (margin["left"], margin["top"] + chart_h/2, chart_w/2, chart_h/2, "#F4F6F9", 0.3),  # bottom-left
        (margin["left"] + chart_w/2, margin["top"] + chart_h/2, chart_w/2, chart_h/2, "#F4F6F9", 0.4),  # bottom-right
    ]
    for x, y, w, h, fill, op in quadrants:
        elems.append(f'<rect x="{x}" y="{y}" width="{w}" height="{h}" fill="{fill}" opacity="{op}"/>')

    # Center cross
    elems.append(f'<line x1="{margin["left"] + chart_w/2}" y1="{margin["top"]}" x2="{margin["left"] + chart_w/2}" y2="{margin["top"] + chart_h}" stroke="#514B78" stroke-width="0.6" stroke-dasharray="2,2" stroke-opacity="0.4"/>')
    elems.append(f'<line x1="{margin["left"]}" y1="{margin["top"] + chart_h/2}" x2="{margin["left"] + chart_w}" y2="{margin["top"] + chart_h/2}" stroke="#514B78" stroke-width="0.6" stroke-dasharray="2,2" stroke-opacity="0.4"/>')

    # Outer frame
    elems.append(f'<rect x="{margin["left"]}" y="{margin["top"]}" width="{chart_w}" height="{chart_h}" fill="none" stroke="#514B78" stroke-width="0.8"/>')

    # X-axis labels
    elems.append(f'<text x="{margin["left"]}" y="{svg_h - margin["bottom"] + 18}" text-anchor="start" font-family="Inter,InterVN,sans-serif" font-size="8" font-weight="600" fill="#645B76">{escape(x_axis.get("low", "Thấp"))}</text>')
    elems.append(f'<text x="{margin["left"] + chart_w}" y="{svg_h - margin["bottom"] + 18}" text-anchor="end" font-family="Inter,InterVN,sans-serif" font-size="8" font-weight="600" fill="#645B76">{escape(x_axis.get("high", "Cao"))}</text>')
    elems.append(f'<text x="{margin["left"] + chart_w/2}" y="{svg_h - margin["bottom"] + 35}" text-anchor="middle" font-family="Inter,InterVN,sans-serif" font-size="9" font-weight="700" fill="#2A1A4A" letter-spacing="0.05em">{escape(x_axis.get("label", "Probability").upper())}</text>')

    # Y-axis labels (rotated)
    y_low_x = margin["left"] - 12
    y_high_x = margin["left"] - 12
    elems.append(f'<text x="{y_low_x}" y="{margin["top"] + chart_h - 4}" text-anchor="end" font-family="Inter,InterVN,sans-serif" font-size="8" font-weight="600" fill="#645B76">{escape(y_axis.get("low", "Thấp"))}</text>')
    elems.append(f'<text x="{y_low_x}" y="{margin["top"] + 10}" text-anchor="end" font-family="Inter,InterVN,sans-serif" font-size="8" font-weight="600" fill="#645B76">{escape(y_axis.get("high", "Cao"))}</text>')
    # Y label rotated
    label_y = margin["top"] + chart_h/2
    elems.append(f'<text x="{margin["left"] - 35}" y="{label_y}" text-anchor="middle" font-family="Inter,InterVN,sans-serif" font-size="9" font-weight="700" fill="#2A1A4A" letter-spacing="0.05em" transform="rotate(-90, {margin["left"] - 35}, {label_y})">{escape(y_axis.get("label", "Impact").upper())}</text>')
    ...
    qlabel_colors = ["#645B76", "#E13453", "#645B76", "#514B78"]
    ...
    for sc in scenarios:
        ...
        color = type_colors.get(sc.get("type", "base"), "#514B78")
        # Dot
        elems.append(f'<circle cx="{x_pos:.1f}" cy="{y_pos:.1f}" r="6" fill="{color}" stroke="#F4F6F9" stroke-width="1.5"/>')
        # Label
        prob_text = ""
        if sc.get("probability"):
            prob_text = f' ({escape(sc["probability"])})'
        elems.append(f'<text x="{x_pos:.1f}" y="{y_pos - 11:.1f}" text-anchor="middle" font-family="Inter,InterVN,sans-serif" font-size="8" font-weight="700" fill="#2A1A4A">{escape(sc["name"])}{prob_text}</text>')
...
def risk_radar(
    ...
    for level in [0.25, 0.5, 0.75, 1.0]:
        ...
        elems.append(f'<polygon points="{" ".join(points)}" fill="none" stroke="#514B78" stroke-width="0.5" stroke-opacity="{opacity}"/>')
    ...
    for i in range(n):
        ...
        elems.append(f'<line x1="{cx}" y1="{cy}" x2="{ex:.1f}" y2="{ey:.1f}" stroke="#514B78" stroke-width="0.4" stroke-opacity="0.2"/>')

    # Target polygon (background, accent dashed)
    if show_target and any(d.get("target") is not None for d in dimensions):
        ...
        elems.append(f'<polygon points="{" ".join(target_points)}" fill="#16633C" fill-opacity="0.08" stroke="#16633C" stroke-width="1" stroke-dasharray="3,2"/>')

    # Current polygon (indigo filled)
    current_points = []
    for i, d in enumerate(dimensions):
        angle = -math.pi/2 + (2 * math.pi * i / n)
    ...
    elems.append(f'<polygon points="{" ".join(current_points)}" fill="#514B78" fill-opacity="0.25" stroke="#2A1A4A" stroke-width="1.5"/>')
    ...
    for i, d in enumerate(dimensions):
        ...
        px = cx + r * math.cos(angle)
        py = cy + r * math.sin(angle)
        elems.append(f'<circle cx="{px:.1f}" cy="{py:.1f}" r="2.5" fill="#2A1A4A"/>')
    ...
    for i, d in enumerate(dimensions):
        ...
        if abs(math.cos(angle)) < 0.3:
            ...
            anchor = "end"
        elems.append(f'<text x="{lx:.1f}" y="{ly:.1f}" text-anchor="{anchor}" font-family="Inter,InterVN,sans-serif" font-size="8" font-weight="700" fill="#2A1A4A" dominant-baseline="middle">{escape(d.get("label", ""))}</text>')

    # Legend
    legend_html = f"""
    <div class="viz-radar-legend">
      <div class="viz-radar-legend-item">
        <div class="viz-radar-legend-swatch" style="background: #2A1A4A;"></div>
    ...
    if show_target and any(d.get("target") is not None for d in dimensions):
        legend_html += f"""
      <div class="viz-radar-legend-item">
        <div class="viz-radar-legend-swatch" style="background: #16633C;"></div>
...
# ============================================================
# CAPSULE VERDICT + SCENARIO OUTLOOK  (equity-fit, flat editorial)
# Colors use source-palette hex so the brand engine remaps them.
# ============================================================
...
.cv{background:#2A1A4A;color:#EBEFF4;margin:0 -24mm;min-height:289mm;
 padding:20mm 26mm 16mm;display:flex;flex-direction:column;justify-content:space-between;
 font-family:'Inter','InterVN',sans-serif;}
.cv-head{display:flex;justify-content:space-between;align-items:flex-start;
 border-bottom:0.8px solid rgba(235,239,244,0.22);padding-bottom:7mm;}
.cv-tk{font-family:'PFD','PFDVN',serif;font-size:44pt;font-weight:700;line-height:0.92;color:#EBEFF4;}
.cv-meta{display:block;font-size:9.5pt;color:rgba(235,239,244,0.6);margin-top:2.5mm;letter-spacing:0.02em;}
.cv-chips{text-align:right;white-space:nowrap;}
.cv-stance{display:inline-block;color:#EBEFF4;font-weight:700;font-size:11pt;letter-spacing:0.02em;padding:3mm 5mm;border-radius:3px;}
.cv-risk{display:block;font-size:8.6pt;color:rgba(235,239,244,0.7);margin-top:3mm;}
.cv-eyebrow{font-size:8.6pt;letter-spacing:0.16em;text-transform:uppercase;color:rgba(235,239,244,0.5);margin-bottom:8mm;}
.cv-lockup{display:flex;align-items:center;gap:12mm;margin-bottom:10mm;}
.cv-mega{font-family:'PFD','PFDVN',serif;font-weight:700;font-size:152pt;line-height:0.74;letter-spacing:-0.04em;white-space:nowrap;}
.cv-mega .u{font-size:80pt;color:#EBEFF4;}
.cv-lockcap{max-width:104mm;}
.cv-lockcap-lead{font-size:8.6pt;letter-spacing:0.12em;text-transform:uppercase;color:rgba(235,239,244,0.56);margin-bottom:3mm;}
.cv-lockcap-main{font-size:14.5pt;line-height:1.42;color:#EBEFF4;font-weight:500;}
.cv-lockcap-main strong{color:#d8a948;font-weight:700;}
.cv-lockcap-sub{font-family:'JBM','JBMVN',monospace;font-size:10pt;color:rgba(235,239,244,0.58);margin-top:4mm;}
.cv-thesis{font-family:'PFD','PFDVN',serif;font-size:21pt;line-height:1.42;color:#EBEFF4;max-width:166mm;}
.cv-metrics{display:flex;}
.cv-tile{flex:1;padding:0 6mm;border-right:0.7px solid rgba(235,239,244,0.16);}
.cv-tile:first-child{padding-left:0;}
.cv-tile:last-child{border-right:none;padding-right:0;}
.cv-val{font-family:'JBM','JBMVN',monospace;font-size:20pt;font-weight:600;line-height:1;color:#EBEFF4;}
.cv-lab{font-size:8.8pt;color:#EBEFF4;margin-top:2.6mm;font-weight:600;}
.cv-note{font-size:7.7pt;color:rgba(235,239,244,0.56);margin-top:1.2mm;line-height:1.3;}
.cv-signal{border-top:0.8px solid rgba(235,239,244,0.22);padding-top:6mm;font-size:9.6pt;color:#EBEFF4;line-height:1.5;}
.cv-sk{display:block;font-size:7.6pt;letter-spacing:0.1em;text-transform:uppercase;color:rgba(235,239,244,0.7);margin-bottom:1.8mm;}
</style>
"""

def capsule_verdict(ticker, meta, stance_label, stance_hex, risk_label,
                    mega_value, mega_unit, stat_lead, stat_main, stat_sub,
                    thesis, metrics, signal, mega_accent="#c8972e",
                    kicker="StockLPT Research · Quan điểm doanh nghiệp"):
    """Full-page magazine hero. 4 blocks evenly distributed (head / hero / metrics / signal)."""
    tiles = ""
    for m in metrics:
        tiles += (
            '<div class="cv-tile">'
            f'<div class="cv-val">{escape(str(m.get("value","")))}</div>'
            f'<div class="cv-lab">{escape(m.get("label",""))}</div>'
            f'<div class="cv-note">{escape(m.get("note",""))}</div>'
            '</div>'
        )
    return (
        _CAPSULE_CSS +
        '<section class="cv-page"><div class="cv">'
        '<div class="cv-head">'
        f'<div><span class="cv-tk">{escape(ticker)}</span>'
        f'<span class="cv-meta">{escape(meta)}</span></div>'
        '<div class="cv-chips">'
        f'<span class="cv-stance" style="background:{stance_hex};">{escape(stance_label)}</span>'
        f'<span class="cv-risk">{escape(risk_label)}</span>'
        '</div></div>'
        '<div class="cv-block">'
        f'<div class="cv-eyebrow">{escape(kicker)}</div>'
        '<div class="cv-lockup">'
        f'<div class="cv-mega"><span class="d" style="color:{mega_accent};">{escape(str(mega_value))}</span>'
        f'<span class="u">{escape(mega_unit)}</span></div>'
        '<div class="cv-lockcap">'
        f'<div class="cv-lockcap-lead">{escape(stat_lead)}</div>'
        f'<div class="cv-lockcap-main">{stat_main}</div>'
        f'<div class="cv-lockcap-sub">{escape(stat_sub)}</div>'
        '</div></div>'
        f'<div class="cv-thesis">{escape(thesis)}</div>'
        '</div>'
        f'<div class="cv-metrics">{tiles}</div>'
        f'<div class="cv-signal"><span class="cv-sk">Tín hiệu theo dõi</span>{escape(signal)}</div>'
        '</div></section>'
    )
...
_SCENARIO_CSS = """
<style>
.so{margin:7mm 0 8mm;font-family:'Inter','InterVN',sans-serif;page-break-inside:avoid;}
.so-title{font-family:'PFD','PFDVN',serif;font-size:13pt;font-weight:700;color:#221A34;margin-bottom:1mm;}
.so-sub{font-size:9pt;color:#645B76;margin-bottom:5mm;}
.so-grid{display:flex;gap:7mm;}
.so-col{flex:1;}
.so-rule{height:3px;width:100%;margin-bottom:4mm;border-radius:2px;}
.so-h{display:flex;justify-content:space-between;align-items:baseline;margin-bottom:3mm;}
.so-name{font-family:'PFD','PFDVN',serif;font-size:12pt;font-weight:700;color:#221A34;}
.so-prob{font-family:'JBM','JBMVN',monospace;font-size:11pt;font-weight:700;}
.so-thesis{font-size:9.4pt;line-height:1.5;color:#221A34;margin-bottom:4mm;}
.so-row{font-size:8.7pt;line-height:1.45;color:#221A34;padding:2.6mm 0;border-top:0.5px solid rgba(43,43,43,0.12);}
.so-k{display:block;font-size:7.1pt;letter-spacing:0.05em;text-transform:uppercase;color:#645B76;margin-bottom:0.8mm;}
</style>
"""

def scenario_outlook(scenarios, title="", subtitle=""):
    """Equity bull/base/bear, flat editorial (no winner/loser framing).
    scenarios: list of {name, tone('up'|'flat'|'down'), prob, thesis, trigger, implication}."""
    tone_hex = {"up": "#21B36A", "down": "#E13453", "flat": "#645B76"}
    cols = ""
    for s in scenarios:
        c = tone_hex.get(s.get("tone", "flat"), "#645B76")
        cols += (
            '<div class="so-col">'
            f'<div class="so-rule" style="background:{c};"></div>'
            '<div class="so-h">'
            f'<span class="so-name">{escape(s.get("name",""))}</span>'
            f'<span class="so-prob" style="color:{c};">{escape(str(s.get("prob","")))}</span>'
            '</div>'
            f'<div class="so-thesis">{escape(s.get("thesis",""))}</div>'
            f'<div class="so-row"><span class="so-k">Điều kiện kích hoạt</span>{escape(s.get("trigger",""))}</div>'
            f'<div class="so-row"><span class="so-k">Hệ quả EPS / định giá</span>{escape(s.get("implication",""))}</div>'
            '</div>'
        )
    head = f'<div class="so-title">{escape(title)}</div>' if title else ""
    sub = f'<div class="so-sub">{escape(subtitle)}</div>' if subtitle else ""
    return _SCENARIO_CSS + f'<div class="so">{head}{sub}<div class="so-grid">{cols}</div></div>'
