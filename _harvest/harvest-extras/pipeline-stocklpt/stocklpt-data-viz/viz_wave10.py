"""
StockLPT Wave 10 - Composite Poster Layouts
==========================================
...
def dashboard_grid(
    ...
    cells: [{
        ...
        "highlight": True,             # optional - nền accent nhẹ
    }]
...
def narrative_strip(
    ...
    panels: [{
        ...
        "color": "#2A1A4A",       # optional override
    }]
...
<svg viewBox="0 0 {panel_w} {panel_h}" xmlns="http://www.w3.org/2000/svg" class="vw-strip-panel-svg">
  <text x="{PAD}" y="14" font-family="{FONTS.SANS}" font-size="7.5"
        font-weight="700" fill="{COLORS.INDIGO}" letter-spacing="0.12em">{label}</text>
  ...
  <text x="{PAD}" y="36" font-family="{FONTS.MONO}" font-size="16"
        font-weight="700" fill="{COLORS.INDIGO}">{value}</text>
  {chart_html}
</svg>
''')
...
.vw-dash-cell-hl {
  background: rgba(22,99,60,0.06);
  border-color: rgba(22,99,60,0.55);
}
.vw-dash-label {
  font-family: 'Inter','InterVN',sans-serif;
  font-size: 6.5pt;
...
.vw-strip-panel-svg {
  ...
  height: auto;
  background: white;
  border: 0.6px solid var(--rule-soft, rgba(22,99,60,0.20));
  border-radius: 3px;
}
.vw-strip-caption {
  display: flex;
...
/* ============ INFOGRAPHIC PANEL ============ */
.vw-infographic {
  background: var(--indigo, #2A1A4A);
  border-radius: 6px;
  padding: 22px 24px 22px 24px;
  margin: 16px 0;
  page-break-inside: avoid;
  color: var(--ivory, #EBEFF4);
}
.vw-info-title {
  font-family: 'PFD','PFDVN','Lora',Georgia,serif;
  font-size: 11pt;
  font-weight: 600;
  color: var(--accent-300, #3C8C5E);
  margin-bottom: 2px;
}
.vw-info-sub {
  font-family: 'Inter','InterVN',sans-serif;
  font-size: 8pt;
  color: rgba(235,239,244,0.65);
  margin-bottom: 14px;
  line-height: 1.4;
  font-style: italic;
}
...
.vw-info-facts {
  ...
  gap: 12px;
  border-left: 0.8px solid rgba(22,99,60,0.40);
  padding-left: 22px;
}
.vw-info-fact {
  display: flex;
  ...
  align-items: baseline;
  padding-bottom: 10px;
  border-bottom: 0.5px solid rgba(235,239,244,0.12);
}
