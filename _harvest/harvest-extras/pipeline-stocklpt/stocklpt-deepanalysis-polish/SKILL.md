---
name: stocklpt-deepanalysis-polish
description: "Polish skill biến bài phân tích sâu/xã luận tài chính của StockLPT Research thành PDF editorial-grade premium - cảm giác đọc tạp chí kinh tế cao cấp (The Economist, Stratechery, Atlantic). CHỈ phục vụ deep analysis: layout 3-stage (cover → TOC → content), drop cap, header strip per-page, body breathable trên paper warm, Spectral + Be Vietnam Pro + IBM Plex Mono. LUÔN dùng skill này khi user paste nội dung essay phân tích nhiều Phần (Phần I/II/III...), có lập luận dài, KHÔNG phải báo cáo phiên giao dịch, hoặc khi user nói 'polish analysis', 'publish deep', '/stocklpt-deep', 'xuất pdf bài phân tích sâu', 'làm pdf editorial StockLPT', 'deep analysis polish'. KHÔNG dùng cho daily report - dùng stocklpt-dailyreport-polish thay vào đó."
---

# StockLPT Deep Analysis Polish Skill

Biến text bài phân tích sâu / xã luận tài chính thành PDF editorial-grade premium theo chuẩn StockLPT Research. Dành riêng cho bài phân tích sâu, có lập luận dài, cấu trúc essay nhiều Phần. Cho daily report dùng `stocklpt-dailyreport-polish` thay vào đó.

**Triết lý design:** PDF đọc trên màn hình (digital publish). Layout breathable: line-height 1.78, font 10.5pt, gap giữa paragraph, drop cap subtle, header strip per-page, magazine-style TOC. Body trên paper warm `#F4F6F9`. Mỗi Phần luôn bắt đầu trang mới.

**Pipeline đầy đủ:**
```
text gốc 
  -> [stocklpt-content-refine: chải chuốt từ ngữ] 
  -> text sạch 
  -> [stocklpt-data-viz: build viz cho key data points] 
  -> [skill này: layout editorial + render PDF]
```

Skill này CHỈ làm visual layout. Nếu thấy content còn nhiều Vietlish ("technical fix", "trade-off", "headroom"...), DỪNG và đề xuất user chạy `stocklpt-content-refine` trước.

**Tích hợp data-viz:** Skill `stocklpt-data-viz` cung cấp 6 components SVG/CSS cho bài analysis (gauge, bar_horizontal, heatmap, flow_bridge, scenario_cards, timeline_horizontal). Khi build CSS, thêm `viz_styles()`:

```python
from render import build_css
from viz import viz_styles  # từ stocklpt-data-viz

css = build_css(date_str=DATE, short_title=SHORT_TITLE) + viz_styles()
```

Sau đó dùng viz functions để build HTML chunks và inject vào body. Xem `stocklpt-data-viz/SKILL.md` để biết chi tiết.
...
2. Body bg dùng `--paper` `#F4F6F9` (KHÔNG `#FFFFFF` thuần). Text dùng `--charcoal` (KHÔNG `#000000`).
3. **Heading**: Spectral (PFD). **Số liệu**: IBM Plex Mono. **Body**: Inter.
4. **PFD italic CHỈ ở signature places** (pull quote, formula, conclusion, h4) - KHÔNG cho dek/intro/repeating elements (giảm AI-feel).
5. **H3 KHÔNG uppercase letterspaced** - dùng Inter normal-case + thin accent underline (uppercase letterspaced là source chính của AI-feel).
6. Bảng màu: Indigo `#2A1A4A`, Indigo-900 `#130B24` (cover), Accent `#16633C`, Slate `#514B78`, Ivory `#EBEFF4`, Paper `#F4F6F9`, Charcoal `#221A34`, Bordeaux `#7A1F35`.
...
10. **Thương hiệu StockLPT (native):** palette Swiss-didactic - ink tím huyền `#2A1A4A`, accent deep forest `#16633C`, giấy off-white mát `#F4F6F9` - và wordmark "STOCKLPT RESEARCH" áp thẳng khi render, không qua lớp chuyển đổi nào.
...
```python
import sys
SKILL_DIR = "/path/to/stocklpt-deepanalysis-polish"
...
**Eyebrow + issue:** ghép từ topic + tháng/năm bằng tiếng Việt. Ví dụ "Ngân hàng & Vĩ mô" + "Số tháng 4 · 2026 · StockLPT Research".
...
<div class="cover-deep">
  ...
  <div class="cover-masthead">
    ...
    <div class="masthead-brand">STOCKLPT RESEARCH</div>
  ...
  <div class="cover-meta-strip">
    <div class="meta-block"><strong>Biên soạn</strong>STOCKLPT Research</div>
...
- `masthead-rule-top`: accent thick rule short (56mm centered) - signature mark
- `masthead-brand`: "STOCKLPT RESEARCH" PFD bold 14pt accent uppercase letterspaced - prominent center
- `masthead-rule-mid`: accent thin rule full-width - separator
- `masthead-meta`: 2-col flex justify-between
  - `meta-left`: category tiếng Việt (e.g. "Ngân hàng & Vĩ mô") accent-300 sans 8.5pt 700
...
Page 1 single block height 297mm + padding 24mm; KHÔNG flex height. Hero stat mono accent 44pt. Bullets accent arrow markers. Hero desc Inter regular (KHÔNG italic).
...
- **`section-roman`**: Roman (I, II, III) hiển thị 76pt accent mờ (opacity 0.10) phía sau title - decorative
- **`section-title-large`**: 20pt PFD bold indigo (KHÔNG 24pt cũ - quá to)
...
```
**Pseudo drop cap** (KHÔNG floated): chữ đầu chỉ tăng size 1.7em + accent color, inline với body. Cleaner, không weird.

### 4c. Body text rich
- `<mark>` cho 3-6 từ key (1-2 lần/đoạn)
- `<span class="t">VCB</span>` cho ticker
- `<strong>` cho emphasis
- **H3**: Inter normal-case 10.5pt 700 + thin accent underline (KHÔNG uppercase letterspaced - source chính của AI-feel cũ)
- **H4**: PFD italic 11.5pt 600 cho micro-subheading (signature italic, hiếm dùng)

### 4d. Pull quotes
**Inline:** `<blockquote><p>...</p></blockquote>` - accent border top/bottom, PFD italic 13pt center.
...
```
Indigo bg + shadow nhẹ + PFD italic 18pt ivory.

### 4e. Callout - 5 levels

**Default:** accent border-left.  
...
**Key:** điểm nhấn quan trọng (1-3 lần/bài), indigo bg + shadow:
```html
...
```
1-2 lần/bài. Mono accent 42pt. Desc Inter regular (KHÔNG italic).
...
Tables editorial: padding tighter 6/10px, header letterspacing 0.10em (KHÔNG 0.16em quá rộng), single accent border bottom (KHÔNG double), row border 0.5px subtle, **bỏ zebra striping** (cleaner).
...
```
Ivory bg + accent borders + shadow nhẹ. PFD italic 13pt (signature italic).
...
<div class="contact-page">
  ...
  <h2 class="contact-title">Đọc thêm cùng STOCKLPT</h2>
  <div class="contact-intro">
    Phân tích này nằm trong chuỗi nghiên cứu chuyên sâu về ngân hàng và vĩ mô của STOCKLPT Research. Để đào sâu hơn từng góc nhìn, thảo luận với chuyên viên, hoặc đăng ký nhận tin định kỳ, hãy liên hệ trực tiếp với đội ngũ.
  </div>

  <div class="contact-cta">
    <div class="cta-eyebrow">Liên hệ đội ngũ STOCKLPT</div>
    <div class="cta-title">Đọc thêm phân tích chuyên sâu hoặc đặt lịch tư vấn 1-1</div>
    <div class="cta-text">
      STOCKLPT Research phát hành báo cáo định kỳ về ngân hàng, vĩ mô, thị trường vốn. Nhà đầu tư tổ chức và cá nhân chuyên nghiệp có thể đặt lịch trao đổi trực tiếp với analyst chủ trì để đào sâu các kịch bản trong báo cáo.
    </div>
    <div class="cta-grid">
      <div class="cta-item">
        <div class="cta-label">Email</div>
        <div class="cta-value">contact@stocklpt.vn</div>
      </div>
      <div class="cta-item">
        <div class="cta-label">Website</div>
        <div class="cta-value">stocklpt.vn/research</div>
      </div>
      <div class="cta-item">
        <div class="cta-label">Đặt lịch tư vấn</div>
        <div class="cta-value">stocklpt.vn/book</div>
      </div>
    </div>
  </div>
  ...
  <div class="contact-section">
    ...
    <p>Toàn bộ phân tích là kịch bản giả định "nếu... thì..." - <strong>không phải dự báo, không phải khuyến nghị mua bán cổ phiếu nào</strong>. Biên soạn: STOCKLPT Research. Nhà đầu tư tự chịu trách nhiệm về quyết định đầu tư của mình.</p>
  </div>

  <div class="contact-signature">
    STOCKLPT Research · © 2026
  </div>
</div>
```
...
- Eyebrow accent + h2 PFD bold 28pt + intro slate sans
- **CTA box indigo bg** highlighted với 3-col grid (Email / Website / Đặt lịch tư vấn) - **đây là phần quan trọng nhất**, encourage user contact
- Sources section + Disclaimer section - paper bg, accent top border
- Signature footer centered

CTA contact info là placeholder - dễ customize cho mỗi bài (substitute `contact@stocklpt.vn`, `stocklpt.vn/research`, `stocklpt.vn/book` thành thông tin thật).
...
render_pdf(html, "/mnt/user-data/outputs/STOCKLPT_THONG_TU_22_20260425.pdf")
```

**Naming convention:** `STOCKLPT_[SLUG]_YYYYMMDD.pdf`
...
3. **Cover full bleed**: page 1 dùng `height: 297mm` block + `@page :first { background-color: #130B24 }`.
...
8. **Body bg paper**: `#F4F6F9` editorial feel, không clash cover indigo.
...
- [ ] Mỗi Phần có Roman numeral bg + dek + pseudo drop cap (1.7em accent, KHÔNG floated)
- [ ] H3 KHÔNG uppercase letterspaced (Inter normal-case + thin accent underline)
...
- Content là daily report (có "phiên", "VN-Index", bảng giá Mã/Phiên/GTGD) → DỪNG, dùng `stocklpt-dailyreport-polish`
- Content còn nhiều Vietlish/AI-style → đề xuất user chạy `stocklpt-content-refine` trước rồi quay lại polish
...
```
4 types với colors: accent (note), slate (methodology), red (warning), green (tip).
...
css = build_full_css(date_str="27/04/2026", short_title="STOCKLPT RESEARCH") + viz_styles()
