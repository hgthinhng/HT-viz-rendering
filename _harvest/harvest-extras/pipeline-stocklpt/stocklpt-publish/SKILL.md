---
name: stocklpt-publish
description: Orchestrator skill - pipeline xuất bản end-to-end cho mọi nội dung StockLPT. LLM (Opus đang chạy skill này) đọc content, classify archetype (banking_regulatory, macro_monetary, sector_deep_dive, equity, fixed_income, fx, commodities, esg, ma, earnings, daily_report), decide pipeline (refine? viz? polish target?), rồi call orchestrator.py execute. Code Python chạy được programmatically (build_plan, execute_plan, render_pdf). Coordinate 4 child skills (stocklpt-content-refine, stocklpt-data-viz catalog 33+ component, stocklpt-deepanalysis-polish, stocklpt-dailyreport-polish). LUÔN dùng skill này khi user muốn pipeline đầy đủ end-to-end thay vì gọi từng skill riêng, hoặc khi user nói '/stocklpt-publish', 'stocklpt publish', 'xuất bản bài StockLPT', 'biến thành PDF StockLPT hoàn chỉnh', 'polish full pipeline', 'make StockLPT PDF'. KHÔNG dùng khi user chỉ muốn 1 bước cụ thể (chỉ refine, chỉ build chart, chỉ polish layout đã có) - dùng child skill tương ứng.
---

# StockLPT Publish - Orchestrator Skill

Skill điều phối pipeline xuất bản StockLPT end-to-end. Triết lý 2 tầng:
...
## Thương hiệu StockLPT

Toàn pipeline dùng một thương hiệu duy nhất: **StockLPT** (Swiss didactic, tông trầm-sang - ink tím huyền `#2A1A4A`, accent deep forest `#16633C`, positive emerald `#21B36A`, đỏ `#E13453`, giấy off-white mát `#F4F6F9`). Palette, wordmark ("STOCKLPT RESEARCH/ADVISORY") và prefix filename PDF đều cố định StockLPT.

Palette nằm native trong `viz.py`/`_tokens.py` và CSS render nên `render_pdf` áp thẳng, không qua lớp chuyển đổi nào. `_brand.py` chỉ còn là lớp tương thích: `current_brand()`, `set_brand()`, `list_brands()`, `get_brand()` vẫn gọi được nhưng luôn trả về StockLPT.
...
pdf_path, plan = publish(content, decision)
# -> .../outputs/STOCKLPT_<slug>_<date>.pdf
```

---

## Dữ liệu FiinQuant (có điều kiện)

`fiinquant_adapter.py` map dữ liệu thị trường -> params viz. **Gate năng lực ở tầng agent, KHÔNG ở code:**
trước khi dùng, agent kiểm tra tài khoản có MCP FiinQuant (paid) không. Có thì pull live đưa dict thô vào adapter; không thì nhận cùng shape dict từ JSON nhập tay. Hai đường dùng chung adapter nên thiếu FiinQuant vẫn chạy, không vỡ. Hàm: `snapshot_to_capsule_metrics`, `peers_to_scatter`, `rows_to_bar`, `prices_to_series`, `vnnum`.

Tự động hoá định kỳ (daily sau ATC) chỉ bật khi có FiinQuant; nếu không, để người dùng nạp số.
...
Đọc `archetypes/<name>/README.md` của child skill `stocklpt-data-viz` để xem chart families + composition pattern.

### Step 2 - Pick polish_target

- `daily_report` archetype → `polish_target = "daily"` (stocklpt-dailyreport-polish)
- Mọi archetype khác → `polish_target = "deep"` (stocklpt-deepanalysis-polish)
...
"back_cover": {
    ...
    # Default StockLPT (polish skill hardcode, LLM không cần fill):
...
Xem catalog spec: `stocklpt-data-viz/catalog/cover_deep_page.md`, `stocklpt-data-viz/catalog/back_cover_cta.md`.

### Step 4 - Build viz_plan

Skim `catalog/INDEX.md` của stocklpt-data-viz, shortlist 5-10 component matching content. Deep-read 5-10 spec. Compose sequence theo composition pattern của archetype tương ứng.
...
```python
import sys
sys.path.insert(0, "/path/to/stocklpt-publish")
...
Cả `stocklpt-dailyreport-polish` và `stocklpt-deepanalysis-polish` đều export module tên `render`. Switch giữa 2 pipelines phải reset cache:
