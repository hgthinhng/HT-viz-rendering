...
chart.setOption({
  ...
  xAxis: { type: 'value', name: '%', axisLabel: { ...TYPOGRAPHY.axisLabel, formatter: (v) => fmtPercent(v, { decimals: 0 }) }, splitLine: { lineStyle: { color: PALETTE.gridline } } },
  ...
  series: [
    {
      ...
      renderItem: (params, api) => {
        ...
        return { type: 'line', shape: { x1, y1: y, x2, y2: y }, style: { stroke: PALETTE.neutralLight, lineWidth: 4 } };
      },
    ...
    },
    {
      ...
      itemStyle: { color: PALETTE.neutralMid },
      data: rows.map((r) => r.before),
      label: { show: true, formatter: (p) => fmtPercent(p.value, { decimals: 1 }), position: 'top', ...TYPOGRAPHY.dataLabel, color: PALETTE.textSecondary },
    },
    {
