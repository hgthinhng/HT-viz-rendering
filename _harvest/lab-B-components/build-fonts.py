#!/usr/bin/env python3
"""
build-fonts.py — tải font canonical StoiX (Fraunces/EB Garamond/Inter/JetBrains
Mono) từ Google Fonts, giữ lại đúng 2 unicode-range (latin + vietnamese; bỏ
latin-ext vì đó là phạm vi Trung/Đông Âu không cần cho nội dung tiếng Việt
thuần — cắt khoảng 57% dung lượng font so với giữ cả 3 subset), base64 hoá,
và sinh fonts-embedded.css để nhúng thẳng vào HTML — chạy được OFFLINE, không
phụ thuộc CDN Google Fonts lúc gửi khách.
...
# 700 thêm cho Fraunces/EB Garamond/JetBrains Mono: rất nhiều rule trong
# components.css xin font-weight:700 trên 3 họ này (h2.sec-title, gate-num,
# rk-idx, v.v.) — nhúng thêm 700 rẻ hơn nhiều so với sửa tay ~30 dòng CSS về
# 600 cho khớp weight đã nhúng.
FAMILY_QUERY = (
...
ORDER_FAM = ["Fraunces", "EB Garamond", "Inter", "JetBrains Mono"]
