...
// index trỏ vào đúng vị trí trong series.rows (0-based). Nhãn được wrap tự động bằng
// wrapLabel() của fmt.mjs (annotation TỰ DO, không nằm trên trục, nên wrap ở tầng hiển
// thị vẫn đúng chỗ -- khác nguyên tắc "rút gọn ở tầng dữ liệu" áp cho nhãn TRÊN TRỤC).
const events = [
...
// lớp chú thích: vẽ SAU khi có toạ độ pixel thật (convertToPixel hoạt động đúng ở chế độ
// ssr:true). setOption lần 2 chỉ thêm `graphic`, KHÔNG xoá series đã có.
const graphics = [];
events.forEach((ev, i) => {
