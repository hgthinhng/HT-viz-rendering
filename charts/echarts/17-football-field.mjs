...
export const MAC_DINH = {
  ...
  currentPrice: 42, // giá thị trường hiện tại: 1 mốc tham chiếu riêng, không phải 1 hàng trong lượt xếp hạng nên không đi qua schema hàng
};

/** Tra ve OBJECT OPTION thuan, cong 1 truong noi bo `_veSauLayout(chart)` cho lop chu
 * thich (vung hoi tu, vach gia thi truong) -- can toa do PIXEL THAT sau khi truc da
 * layout. Truc x o day KHAI TUONG MINH min/max (tinh tu chinh du lieu, xem axisMin/
 * axisMax duoi day) nen VE MAT TOAN co the doi ra cong thuc tuyen tinh thuan tuy, nhung
 * van dung co che _veSauLayout CHUNG voi 13/18 de mot loi tinh DUY NHAT (chart.convertToPixel
 * that) chiu trach nhiem, khong phai bao tri hai duong tinh song song co the troi nhau. */
export function option(params) {
  const { series: seriesGoc, currentPrice } = params;
  // schema đòi mỗi hàng có 1 value số thực (status mặc định co_that) -- dùng trung điểm
  // dải làm giá trị đại diện cho hàng, low/high vẫn giữ nguyên để vẽ dải. Xay OBJECT MOI,
  // KHONG gan de len params.series.rows: params co the la MAC_DINH dung chung, sua tai
  // cho se lam lan goi option() thu hai tren cung object bi lech du lieu dau vao.
  ...
  return {
    ...
    // mảng rồi setOption MỘT lần duy nhất, do renderStatic()/mountLive() thực hiện SAU khi
    // truc da layout. Da thu tach 3 lan setOption rieng, noi tiep bang `chart.getOption()
    // .graphic` nhu tien le markLine o 06-tornado.mjs -- KHONG ap dung duoc cho graphic:
    // getOption().graphic tra ve dang boc `[{elements:[...]}]` chu khong phai mang phang
    // da dua vao, spread lai tao ra 1 phan tu group lan voi phan tu roi rac cung cap,
    // ECharts am tham bo qua cac phan tu them sau (da tu bat loi nay bang anh chup that:
    // vach gia thi truong va 2 chu giai mau bien mat hoan toan). convertToPixel van dung
    // duoc ngay sau setOption dau tien (chi can truc da co, khong can graphic da ton tai),
    // nen tinh toan truoc, gom lai, roi set 1 lan trong _veSauLayout.
    _veSauLayout(chart) {
      const graphics = [];
      ...
      // vạch giá thị trường hiện tại: z cao nhất để luôn nổi trên các dải và vùng hội tụ.
      // Màu ink trung tính (KHÔNG dùng negative): giá thị trường hiện tại không mang nghĩa
      // "xấu/rủi ro" tự thân, tô đỏ dễ vô tình đọc thành "giá thị trường là tin xấu" -- đây
      // là 1 mốc tham chiếu chứ không phải 1 tín hiệu cảnh báo, khác cách dùng negative cho
      // kịch bản bất lợi.
      {
      ...
      // 2 chú giải màu ở chân chart -- annotation-first, không dùng legend ECharts vì chỉ
      // có 2 nhóm màu cố định xuyên suốt mọi hàng, ghi 1 lần dưới chart gọn hơn legend
      // interactive.
      graphics.push(
