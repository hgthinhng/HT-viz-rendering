...
const waccRows = [9.0, 9.5, 10.0, 10.5, 11.0]; // %, trục hàng
const gCols = [2.0, 2.5, 3.0, 3.5, 4.0]; // %, trục cột (tăng trưởng dài hạn)
const matrix = [
...
waccRows.forEach((w, ri) => gCols.forEach((g, ci) => {
  ...
  data.push({
    ...
    // SAU CÙNG bên dưới (z cao hơn mọi ô), xem khối graphics phía dưới.
...
chart.setOption({
  ...
  grid: { left: 76, right: 24, top: 96, bottom: 84 },
  xAxis: {
  ...
  // dùng được SAU khi trục đã tồn tại. Gom hết graphic vào 1 mảng rồi set đúng 1 lần ở
  // dưới (bài học từ preset 17: chart.getOption().graphic trả về dạng bọc khác cấu trúc
  // đưa vào, spread lại để nối thêm graphic làm ECharts âm thầm bỏ qua phần tử thêm sau).
});

// khung base case: 1 rect graphic riêng, vẽ SAU series heatmap nên KHÔNG bị ô kề bên đè
// mất cạnh (khác itemStyle.borderColor đặt trên chính ô, bị 2 ô vẽ sau đè mất 2/4 cạnh,
// đã tự bắt lỗi này bằng ảnh chụp crop cao). fill:none nên không đụng màu nền ô, giữ đúng
// nguyên tắc "chỉ đánh dấu bằng viền, không đổi màu nền" -- đổi nền phá tính liên tục của
// thang màu.
//
// Toạ độ: KHÔNG dùng convertToPixel([baseCol-0.5, baseRow-0.5]) để lấy trực tiếp 2 góc ô
// -- đã tự thử và bắt bằng ảnh chụp crop cao: kết quả lệch nguyên NỬA Ô sang phải-dưới so
// với ô "850" thật (category axis không nội suy nửa-chỉ-số như trục value). Cách đúng đã
// verify bằng cách so với toạ độ <path> thật của chính ô đó trong SVG render: lấy TÂM ô
// base case qua convertToPixel([baseCol, baseRow]) (số nguyên, luôn đúng vì đó chính là
// chỉ số category), rồi lấy khoảng cách tâm-tới-tâm với ô kế bên để suy ra bề rộng/cao 1
// ô -- 2 tâm liền kề cách nhau ĐÚNG BẰNG 1 band width bất kể offset nội bộ của HeatmapView.
