...
## Bảng tra: [Ngành/luận điểm] → [Ẩn dụ vật lý] → [Vì sao đúng cơ chế, không chỉ đẹp]
...
3. **Sản xuất / thép / xi măng** → Nhà máy có ống khói + dây chuyền băng
   tải → Dây chuyền = chuỗi giá trị tuần tự có thể tách khúc chi phí theo
...
6. **Bán lẻ / tiêu dùng** → Giỏ hàng + quầy thu ngân/kệ hàng → Giỏ hàng đầy
   hay vơi ánh xạ trực tiếp sales/basket size, không cần diễn giải thêm.
7. **Bất động sản dân dụng** → Tháp căn hộ + cần cẩu tháp đang thi công →
   Số tầng đã đổ bê tông = tiến độ bàn giao thật, cần cẩu = vốn đang đổ
   vào, dừng cẩu = dừng dòng tiền dự án.
8. **Điện / năng lượng tái tạo** → Cột điện cao thế + turbine gió → Cột
   điện = mạng truyền tải (bottleneck vật lý thật của ngành điện gió,
   không phải ẩn dụ), turbine quay/đứng = công suất phát thực tế.
9. **Phán quyết đầu tư / khuyến nghị MUA-BÁN-GIỮ** → Cán cân hai đĩa
   (balance scale) → Cân là ẩn dụ ĐÚNG BẢN CHẤT: quyết định đầu tư luôn là
   phép so trọng số hai phía bằng chứng, độ nghiêng thật sự phản ánh chênh
   lệch trọng số, không phải trang trí tuỳ hứng.
10. **Cấu trúc chi phí / biên lợi nhuận** → Lát cắt ngang máy móc/dây
    chuyền (cutaway) hiện từng lớp chi phí xếp chồng → Cutaway cho thấy
    TỪNG LỚP ẩn bên trong theo đúng thứ tự vật lý (nguyên liệu → nhân công
    → khấu hao → SG&A), khớp cấu trúc P&L theo lớp.
11. **Chuỗi giá trị / chuỗi cung ứng** → Dây chuyền băng tải nhiều trạm
    nối tiếp → Băng tải = tính chất TUẦN TỰ, PHỤ THUỘC LẪN NHAU của chuỗi
    giá trị; đứt 1 trạm là toàn chuỗi dừng, đúng rủi ro thật của supply
    chain.
12. **Rào cản gia nhập ngành** → Cổng/hào (gate + moat) quanh một khu vực
    → Hào nước là ẩn dụ kinh điển đúng nghĩa "competitive moat", cổng gác
    thể hiện chi phí/thủ tục gia nhập như giấy phép, vốn tối thiểu.
13. **Kênh chuyển đổi khách hàng (funnel)** → Phễu vật lý nhiều tầng thu
    hẹp dần → Hình phễu = tỷ lệ rơi rụng thật giữa các tầng (awareness →
    lead → khách trả tiền), độ thu hẹp mỗi tầng ánh xạ conversion rate.
14. **Cơ cấu vốn / đòn bẩy tài chính** → Cán cân hoặc trụ chống đỡ có tải
    trọng đặt trên đỉnh (dầm chịu lực) → Đòn bẩy = nghĩa đen (leverage),
    dầm càng dài mà trụ càng mảnh thì rủi ro sập càng thật, đúng cơ chế
    vay nợ.
15. **Chu kỳ ngành / tính chu kỳ (cyclicality)** → Bánh đà hoặc bánh xe
    quay với các mốc trên vành → Vành tròn = tính lặp lại có chu kỳ thật,
    các mốc trên vành = giai đoạn (đáy/đỉnh) đọc được vị trí hiện tại trên
    chu kỳ.
16. **Rủi ro tập trung (concentration risk)** → Một cột/chân trụ duy nhất
    đỡ toàn bộ mái, so với nhiều chân → So sánh trực quan "1 chân vs N
...
18. **Nông nghiệp / thuỷ sản** → Ao nuôi/ruộng theo lô, có chú thích mật
    ...
    ngành, chia lô đúng bằng cách người trong ngành chia thật.
19. **Viễn thông / hạ tầng mạng** → Cột phát sóng (tower) + vùng phủ sóng
    hình tròn đồng tâm → Vùng phủ tròn đồng tâm là ẩn dụ đúng vật lý lan
    truyền tín hiệu (suy hao theo khoảng cách), không phải trang trí.
20. **Dược phẩm / R&D** → Ống nghiệm + phễu lọc theo từng pha thử nghiệm
    (phase funnel dạng phòng thí nghiệm) → Số lượng ứng viên thuốc rơi
    rụng qua Phase I/II/III đúng là một funnel thật của ngành, không phải
    phễu bán hàng mượn tạm.
21. **Hàng không / vận tải hành khách** → Thân máy bay cắt ngang theo
    hạng ghế → Cutaway thân máy bay ánh xạ đúng cấu trúc doanh thu theo
...
## Nguyên tắc khi nào KHÔNG nên dùng ẩn dụ

- **KHÔNG** dùng khi ẩn dụ LÀM SAI LỆCH dữ liệu: nếu độ nghiêng/độ lớn/số
  lượng trong hình không thể ánh xạ trung thực với con số thật (vd: vẽ cán
  cân nghiêng 45 độ trong khi trọng số thực chỉ chênh 2%), thà dùng biểu đồ
  số liệu thường (bar/line) còn hơn ép vào ẩn dụ và làm người đọc suy diễn
  sai độ lớn.
