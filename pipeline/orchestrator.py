...
## Hai làn, và làn quyết định hai bước cuối

`--lan=pdf-so` (mặc định) giữ nguyên đường cũ từng chữ: xuất PDF rồi chạy mười gate
trên cặp HTML cộng PDF.

`--lan=html-song` thì **thứ giao đi chính là file HTML**, không có PDF nào cả. Nên:

    5. CK3       BỎ HẲN. Xuất PDF cho một trang có chart mount lúc chạy là tự chuốc
                 lấy bản PDF thiếu chart, vì WeasyPrint không chạy JavaScript.
    6. GATE      chín gate của `gates_song.mjs`, mở Chromium thật, đo hành vi lúc chạy.

Bước 3 CK2 cũng bỏ ở làn này: ba bản bìa nghiệm thu bằng cách xuất PDF ra so, mà làn
này không xuất PDF.
...
def main() -> int:
    ...
    ap.add_argument("--lan", dest="lan", default="pdf-so", choices=["pdf-so", "html-song"])
    t = ap.parse_args()
    ...
    ten = nguon.stem
    la_song = t.lan == "html-song"
    if la_song:
        # Ca hai buoc nay deu di qua WeasyPrint. Giu chung o lan song la sinh ra artifact
        # THIEU CHART roi de no nam canh ban that trong cung thu muc `ra/`, tuc dung dieu
        # kien de mot nguoi cam nham file di gui. Bo han con hon.
        bo_di = [b for b in ("ck2", "ck3") if b in can_chay]
        can_chay = [b for b in can_chay if b not in ("ck2", "ck3")]
        if t.chi and not can_chay:
            print(f"orchestrator: buoc `{t.chi}` khong ton tai o lan html-song", file=sys.stderr)
            return 1
        if bo_di:
            print(f"lan html-song: bo buoc {', '.join(bo_di)} vi lan nay khong xuat PDF")
    ...
    try:
        ...
        if "dung" in can_chay:
            ...
            for che_do in ("noi-bo", "gui-di"):
                ...
                dung(nguon, ra_html, che_do, t.lan)
                ban[che_do] = ra_html
        ...
        if "gate" in can_chay and not t.bo_qua_gate:
            print("[6/6] GATE" + (" lan html-song, chin gate" if la_song else ""))
            hong = False
            for che_do in ("noi-bo", "gui-di"):
                ra_html = thu_muc_ra / f"{ten}-{che_do}.html"
                ra_pdf = ra_html.with_suffix(".pdf")
                lenh_gate = (
                    ["node", str(REPO / "gates/run.mjs"), str(ra_html), "--lan=html-song"]
                    if la_song
                    else ["node", str(REPO / "gates/run.mjs"), str(ra_html), str(ra_pdf),
    ...
    if la_song and "dung" in can_chay:
        print(f"  Ban giao di:       {ten}-gui-di.html (lan html-song, mo bang trinh duyet)")
    if "hinh" in can_chay:
