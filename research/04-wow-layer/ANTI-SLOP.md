...
| 2 | Tạo độ nổi / phân lớp | **Đã kiểm bằng so ảnh mức byte: `box-shadow` và `text-shadow` không tồn tại trong WeasyPrint 69.0 dưới bất kỳ cú pháp nào (xem FINDINGS.md mục 8)**, kể cả offset cứng blur 0. Bản đắt thật là 2 phần tử thật (khối/chữ nền màu nhạt lệch vị trí bằng `transform`, phần tử chính đè lên bằng `z-index`), hoặc `border` đặc, hoặc `background-color` khối | Copy `var(--shadow-1)` từ tokens.css và tin nó tạo hiệu ứng gì đó trong PDF thật, hoặc `box-shadow` có blur, hoặc gradient nền giả ánh sáng, hoặc `filter: drop-shadow()` | Mở PDF thật (không phải xem trên Chromium), phóng to 300%: nếu thấy có 2 lớp hình học lệch nhau thật (không phải bóng mờ) là đắt; nếu trang trông PHẲNG TUYỆT ĐỐI dù CSS có khai `box-shadow` là dấu hiệu tác giả chưa verify PDF thật, chỉ xem trên trình duyệt |
