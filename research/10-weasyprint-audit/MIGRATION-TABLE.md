...
`illustrations/` (đúng phạm vi được giao) + **1 chỗ đáng chú ý ngoài phạm vi** (nêu để không bị
bỏ sót, không phải việc của agent này) + **1 dòng dọn dẹp không liên quan bẫy WeasyPrint**.
...
## Mức 2: đáng chú ý, ngoài phạm vi sửa của agent này (nêu để không bị bỏ sót)

| # | Chỗ | Bẫy | Kỹ thuật thay thế đề xuất | Ưu tiên | Công ước lượng |
|---|---|---|---|---|---|
| M3 | `samples/report-exec-brief-action-first.html:64`, `h1.verdict` (thuộc vòng nghiên cứu 02, NGOÀI 4 thư mục được giao cho agent này) | `font-size: clamp(1.55rem, 1.1rem + 1.6vw, 2.15rem)` bị bỏ qua hoàn toàn, đo thật ra `12.0pt` - bằng cỡ chữ thân bài, mất hết phân cấp cho dòng kết luận hành động chính | Đổi sang `rem` cố định, ví dụ `font-size: 1.85rem;` (điểm giữa dải clamp cũ, ~29.6px) - không cần responsive vì đây là PDF khổ cố định A4, không phải trang web co giãn | **Cao nếu file này còn được dùng làm mẫu tham chiếu thật** (đã bị cảnh báo từ vòng 08-synthesis, chưa ai sửa) | 1 dòng, nhưng cần đúng người sở hữu vòng 02/08 sửa vì agent này chỉ được ghi vào `research/`+`samples/` mới, không sửa file `samples/` có sẵn của vòng khác |
