...
| 4 | Trạng thái rỗng và trạng thái thiếu dữ liệu trong báo cáo | xong (16 nguồn khảo sát định chế/toà soạn/luật VN gồm IMF/World Bank/BIS/Eurostat/UN/OECD/comps IB/Datawrapper/Liberty Street Economics/FactSet/Thông tư 96 xác nhận qua WebFetch toàn văn, hồ sơ 6 mục cộng 1 mục phát hiện kỹ thuật mới + 1 loại vắng mặt thứ bảy ghi nhận trung thực ngoài sáu loại gốc, 4 mẫu render thật qua WeasyPrint 69.0, 1 lỗi kỹ thuật tự bắt và tự vá bằng 12 biến thể cô lập) | `research/11-empty-states/FINDINGS.md` (mục 0 phát hiện kỹ thuật mới; mục 1 khảo sát 7 định chế/thực hành cho bảng sáu ký hiệu cộng lý do KHÔNG copy nguyên một định chế nào; mục 2 ngưỡng đừng vẽ bảng nữa; mục 3 ba cách vẽ khoảng đứt chart; mục 4 so sánh chuỗi dài ngắn; mục 5 chưa tới kỳ so với quá hạn neo bằng Thông tư 96/2020/TT-BTC Điều 14; mục 6 ghi chú độ phủ) + `research/11-empty-states/ABSENCE-TABLE.md` (bảng tra sáu-cộng-một hàng: ký hiệu, nghĩa, khi dùng, khi đừng, đọc được khi in đen trắng không, cộng nguyên tắc "tần suất cao ép terse, rủi ro cao ép nổi bật" giải thích vì sao 4 hàng đầu chỉ dùng ký tự thuần còn 2 hàng cuối cần thêm viền). 4 mẫu trong `samples/`: `empty-bang-sau-loai-vang-mat.html` (bảng 7 doanh nghiệp hỗn hợp ngành lộ đủ sáu loại vắng mặt thật, cộng khối đối chứng ngưỡng "8 cột 5 trống" bằng số liệu cụ thể 15/35≈43% dưới ngưỡng 50%), `empty-chart-dut-gay-du-lieu.html` (ba biến thể cùng bộ số: nối liền/nét đứt/ngắt-tô-nhạt-nhãn, đối chiếu tường minh với quy ước nét-đứt-là-dự-phóng đã chốt ở vòng 2 để không đánh lừa chính quy ước đó), `empty-so-sanh-chuoi-dai-ngan.html` (18 năm so với 3 năm dữ liệu, phanh phui bẫy chỉ số hoá về gốc xa hơn lịch sử thật - cùng cơ chế lỗi với memory `feedback_baseline_must_be_drawn_inside_the_subgroup` nhưng ở mốc gốc index thay vì baseline thống kê, cộng hai cách thay thế có đánh đổi rõ), `empty-trang-thai-rong-toan-phan.html` (thẻ KPI rỗng toàn phần: "chưa đủ điều kiện tính" trung tính so với "đã quá hạn công bố" cần chú ý, viền `--warn` không phải `--neg` vì quá hạn là tín hiệu quy trình chưa phải kết luận xấu đã xác nhận). Nghiệm thu đủ 3 phép bắt buộc qua WeasyPrint+pymupdf cho cả 4 file, 0 raster, 0 em/en dash (đã tự bắt và sửa 2 chỗ gõ nhầm bao gồm cả trong chính `FINDINGS.md`/`ABSENCE-TABLE.md` lúc mô tả ký tự bị cấm - đổi sang ghi mã U+2014/U+2013 thay vì gõ nguyên ký tự). **Phát hiện kỹ thuật mới, chưa vòng nào ghi nhận, ảnh hưởng tiềm tàng mọi chấm tròn/step-indicator trong repo**: `<span>` (mặc định `display:inline`) dùng trực tiếp làm flex item với `border-radius:50%` bị WeasyPrint 69.0 vẽ thêm một ĐUÔI NHỌN ở một góc thay vì khép kín thành hình tròn - bắt được bằng mở ảnh 400dpi (không lộ qua text round-trip vì không sai chữ nào), cô lập nguyên nhân bằng 12 biến thể tối giản (loại trừ lần lượt: flex lồng nhau, `min-width:0`, đơn vị `mm`, phần tử đứng trước, font Spectral - TẤT CẢ không phải nguyên nhân), xác nhận đúng biến `<span>` mặc định inline là thủ phạm và việc khai tường minh `display:inline-block` trước khi flex blockify (hoặc đổi hẳn sang `<div>`) sửa dứt điểm. Đã sửa `.step-dot` trong `empty-trang-thai-rong-toan-phan.html` và verify lại bằng crop ảnh 400dpi cho kết quả tròn tuyệt đối. Đối chiếu ngược: `.dot-hollow`/`.swatch-dot` có sẵn từ vòng 2 KHÔNG dính lỗi vì tình cờ đã khai `display:inline-block` tường minh - nay có lý do kỹ thuật rõ ràng cho một dòng CSS trước đây tưởng là chi tiết ngẫu nhiên. **Một loại vắng mặt thứ bảy ngoài sáu loại gốc, ghi nhận trung thực thay vì ép vào**: chỉ số phái sinh (P/E trượt bốn quý) chưa đủ độ sâu lịch sử để tính dù từng kỳ input đã công bố đúng hạn đầy đủ - khác cả sáu loại gốc, xử lý bằng chuỗi chấm đếm được (●●○○, KHÔNG phải gauge) cộng câu giải thích tại chỗ thay vì thêm ký hiệu bảy vào bảng chính. |
...
- ~~Ưu tiên cao: audit toàn repo xem có chỗ nào dùng `clamp()`/`min()`/`max()`~~ **ĐÃ XONG (vòng
  ...
  `.no-print`); 1 chỗ TỪNG THẬT SỰ VỠ nằm ngoài phạm vi 4 thư mục, ở
  `samples/report-exec-brief-action-first.html` (`h1.verdict` đo ra 12pt thay vì ~19-26pt dự
  kiến lúc phát hiện) - **đã được vòng khác vá xong (commit `75e5ffb`) ngay trong lúc vòng 4 đang
  chạy**, đo lại xác nhận ra đúng 22,8pt cố định, không còn việc phải làm. Bonus phát hiện cần
  chỉnh lại cách hiểu: `<svg height="auto">` dạng THUỘC TÍNH
...
- ~~Trạng thái rỗng và trạng thái thiếu dữ liệu trong báo cáo~~ **ĐÃ XONG (vòng 4)**: bảng sáu
  ký hiệu, ngưỡng đừng vẽ bảng, ba cách vẽ khoảng đứt chart, so sánh chuỗi dài ngắn, chưa tới kỳ
  so với quá hạn, ghi chú độ phủ - xem dòng vòng 4 ở bảng trên và
  `research/11-empty-states/FINDINGS.md`/`ABSENCE-TABLE.md`.
- **Mới, ưu tiên cao, ảnh hưởng tiềm tàng toàn repo, từ vòng 4 (trạng thái rỗng)**: audit
  `components/`, `charts/`, và mọi `samples/*.html` bằng `grep -rn "border-radius: 50%"` rồi kiểm
  từng chỗ có dùng `<span>` làm flex/inline-flex item mà THIẾU khai tường minh
  `display:inline-block` hay không - WeasyPrint 69.0 vẽ thêm một đuôi nhọn ở một góc cho trường
  hợp này thay vì hình tròn khép kín, chỉ lộ ra khi mở ảnh ở độ phân giải cao, không lộ qua text
  round-trip. Xem `research/11-empty-states/FINDINGS.md` mục 0 để có 12 biến thể cô lập nguyên
  nhân và cách sửa (thêm `display:inline-block` hoặc đổi hẳn sang `<div>`). Đã sửa 1 chỗ tự phạm
  trong chính vòng 4 (`samples/empty-trang-thai-rong-toan-phan.html`), chưa audit phần còn lại
  của repo.
